...
8. Materialize polygons directly or invoke SAM2 with bounded positive/negative points. Store under the
   S11 cloud-candidate directory; never overwrite the S09 authoritative baseline, gold, or a frozen
   package mask. A separately identified non-gold review draft may be composed for S12.
...
10. Compose the best demonstrably improved, hard-QA-passing candidates into a reversible non-gold
    review draft before the human handoff. Apply and validate each label independently, roll back only
    the failing label, preserve the exact S09 baseline and every candidate hash, and then rerun final
    complete-map hard QA. The review report records changed labels, selection reasons, provider votes,
    before/after metrics, and remaining uncertainty. This preselects a starting draft only: it does not
    clear careful review, approve gold, or alter the baseline. Record whether each diagnosis was correct,
    whether its proposed correction was useful, human edit time, changed pixels, final gold hash,
    baseline IoU, and boundary F-score.

...
giving the human a better starting mask. A non-baseline candidate may receive the artifact status
`pre_review_improvement` when it is hard-veto-free, clears a hard-vetoed baseline or has a sufficient
blinded better-than-baseline result, and passes incremental complete-map QA. S11 applies these changes
label-by-label to a reversible non-gold review draft, preserves the S09 baseline, and reruns final
complete-map QA. Any label that fails is rolled back without discarding successful changes elsewhere.
The tournament lifecycle may remain `residual_human_queue`; the review-draft status grants no
autoaccept, gold, training-truth, blocker-clearance, or package-state authority.
...
- `src/maskfactory/autonomy/review_draft.py`: independently selects safe pre-review improvements,
  composes them with per-label rollback, preserves baseline/candidate hashes and provider evidence,
  and requires final complete-map QA before S12/CVAT receives the optimized non-gold draft.
...
- S11 now writes real baseline/local/cloud candidate masks, runs the tournament per label, emits a
  validated lifecycle sidecar, and normally sends the best safe optimized non-gold draft to S12/CVAT.
  It remains residual-only for skipping human review until an exact unrevoked certificate is supplied.

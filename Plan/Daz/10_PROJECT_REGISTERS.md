...
| D-015 | SUPERSEDED: every Pod-local GPU child requires the shared FIFO lease and guarded launcher | cross-session safety and current standing orders | user policy change |
...
| R-014 | Runtime OOM | M | High | shared FIFO lease, typed workload failure, and durable retry/abstain | release lease; never preempt foreign work or weaken quality |
...
| DEP-008 | Shared Pod GPU lease and guarded launcher | stable rendering | mandatory for every Pod-local GPU child |

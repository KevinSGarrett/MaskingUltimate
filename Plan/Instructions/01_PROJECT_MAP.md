...
|---|---|---|---|
| **Spec** | `Plan\00`–`28`, plus approved handoffs | The complete technical blueprint, including autonomous exact-output authority, claim-scoped completion, continuous self-hosted throughput, canonical whole-product integration, the MaskFactory↔ComfyUI bridge, evidence-qualified visual critics, and RunPod asset reconciliation. | Changes only through deliberate spec/owner amendments. Later approved decisions outrank conflicting earlier text and must be recorded in traceability. |
| **Checklist** | `Plan\Items\` (24 parsed phase files, 906 action items) | The spec atomized into checkable build items, each tagged with its phase, governing source, explicit verification, and blockers. | Edit only when the plan itself changes. After editing, run `python tracker.py rebuild` in `Tracker\`. |
...
## 2. The Specification Set (`Plan\00`–`28` plus approved handoffs)
...
| 05 | `05_SYSTEM_ARCHITECTURE.md` | Component architecture, module boundaries, consensus engine, shared-lease-guarded RunPod GPU execution |
| 06 | `06_ENVIRONMENT_AND_INSTALLATION.md` | Hardware plan, exact env builds, every model checkpoint, CVAT deployment |
| — | `DOCKER_RUNTIME_AND_SESSION_USE.md` | Production RunPod and optional local-integration policy; local Docker/WSL/Ollama requires Kevin's exact current-turn request |
...
| 27 | `27_SELF_HOSTED_AUTONOMOUS_LLM_CONTINUOUS_OPERATIONS_SPEC.md` | Continuous CPU supervision, campaign runtime, governed routing, real 25/100/mixed campaigns, recovery, and throughput acceptance |
| 28 | `28_ULTIMATE_MASKING_SYSTEM_E2E_INTEGRATION_AND_COMFYUI_ADOPTION.md` | Canonical product/autonomy convergence, clean full suite, evidence reconstruction, mask-safety semantics, immutable release, and pinned ComfyUI adoption |
...
The parsed checklist contains **906 atomic items across 24 phase files**.
...
to `21_ITEMS_P6_AUTONOMOUS_CORE_AND_CROSS_PROJECT_BRIDGE.md`; Plans 27 and 28
map to Item files 23 and 24. Each item
...
| `tracker.json` | Canonical live state of all 906 items + completion placeholders + DoD + Goals + metrics. |
...
| What proves the entire MaskFactory product—not only its autonomy subsystem—is complete and adopted by ComfyUI? | `Plan\28`, `Items\24_ITEMS_P6_ULTIMATE_MASKING_SYSTEM_E2E_INTEGRATION.md`, and `Instructions\18_ULTIMATE_MASKING_SYSTEM_E2E_INTEGRATION_AND_COMFYUI_ADOPTION.md` |

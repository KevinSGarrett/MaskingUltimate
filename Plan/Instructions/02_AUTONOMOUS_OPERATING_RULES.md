...
Every source receives the same registration metadata. Record exact provenance,
...
**Binding (Kevin mandate — implemented into rules):** Agents are **not
allowed to stop**. Work continuously and autonomously until the **entire
MaskFactory project is fully completed end-to-end**. Auto only; NEVER EC2.
Canonical text: Standing Orders § **CONTINUOUS UNTIL E2E COMPLETE (NO STOP)**.
Re-read mid-flight. Side-chat does **not** relax this.

### Forbidden
- Ending a wave with idle wait for Kevin chat / “awaiting instructions.”
- “No further action,” “parked,” “done for now” while unblocked product work remains.
- Parking after a seal/audit while climbable HARD_QA / RUNTIME / VISUAL /
  package / bridge / Mode A/B / gold-path work is still open.
- Waiting for subagent notifications without **immediately chaining** the next
  highest-value unblocked wave.
- Treating usage-limit, context exhaustion, session/agent death as a project
  pause without durable `nohup` (or equivalent) + handoff + **new agent relaunch**.
- Idling on one `NEEDS KEVIN` lane when other unblocked lanes exist.

### Required
1. After every wave completion → **IMMEDIATELY** chain next highest-value
   unblocked work (`tracker.py next`; parallelize independent lanes).
2. Durable long jobs on RunPod/host (`nohup` / equivalent) so **Cursor agent
   death ≠ climb death**.
3. Stop only for true `NEEDS KEVIN` (credentials / privileged host / external
   authority). Even then: typed evidence + switch to **all other** unblocked lanes.
4. Never weaken STRICT VLM gate or proof tiers to invent a stop condition.

Session procedure detail: `03_SESSION_PLAYBOOK.md` §§2, 4, 5.

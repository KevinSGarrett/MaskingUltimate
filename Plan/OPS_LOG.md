...
## 2026-07-13 - Optimized non-gold autonomy draft made the normal CVAT input

**Result:** PASS - safe residual candidates now improve the human review draft without receiving gold or autoaccept authority.

selection repair:                non-baseline winners that clear a baseline hard veto or have sufficient blinded better-than-baseline evidence can receive pre_review_improvement even without an autonomy certificate
hard-QA boundary:                candidate selection remains hard-veto-free; incremental complete-map QA is mandatory per label and final complete-map QA is mandatory before promotion to S12
rollback behavior:               one failing label rolls back independently while previously validated label improvements remain in the review draft
CVAT handoff:                    S12 already consumes autonomy_review_draft/label_map_part.png when promoted; the repaired selection path now makes that optimized non-gold artifact the normal safe input instead of raw S09
provenance:                      report includes changed labels/pixels, selection reason, baseline and winner scores/evidence, provider participation/votes, uncertainties, hashes, and incremental/final QA
provider scope:                  local Qwen participates on every present label; Gemini/OpenAI/Anthropic participation is recorded when image eligibility, escalation, provider availability, and Kevin-authorized budget permit, with non-participation explicit
learning measurement:            frozen human-gold resolution records now compare S09 and optimized drafts against gold, record machine/human changed pixels, correction pixels avoided, and real review_time_sec
authority unchanged:             humans remain sole gold authority; certificate absence prevents review bypass but no longer prevents safe pre-review improvement; no provider call or gold approval was executed
verification:                    107 focused autonomy/cloud/CVAT-package/production tests passed; 757 full tests passed; Ruff clean; exact Black 26.5.1 clean on changed code
...
## 2026-07-13 - Governed Civitai specialist runtime and stress packs wired into production

**Result:** PASS for assist/review/stress integration; vote promotion remains correctly blocked by zero approved gold.

runtime integration:             all 24 registered auxiliary detectors now have explicit task/class, ROI/gate, output remap, priority, and disabled/shadow/assist/vote policy; checkpoint/config identity is pinned and verified
production path:                 S06 selects and runs specialists sequentially, preserves raw evidence, builds sided hand/foot and hair assists, S07 refines with SAM2, S08 accepts only explicit segmentation seeds, and S09 accepts only certificate-backed 0.05 votes
live draft proof:                img_2ca794d19be9 ran 14/14 selected providers successfully with 11 detections; assisted SAM2 predicted-IoU was .9160/.8755 for hands and .84375/.9077 for feet
false-positive response:         sock-on-bare-leg and headwear-on-hair material errors were observed, preserved, and demoted to shadow/protected-only; unvalidated clothing-top material output also remains shadow
review acceleration:             S12 now packages source-aligned specialist overlays and normalized proposal evidence beside disagreement; the controlled mask-delta adapter stages canonical edits, requires explicit subtract replacement, refuses frozen gold, regenerates derivations, reruns QC-001..010, and cannot approve
stress consumption:              all 22 pose/control packs now produce a deterministic, archive-verified S15 stress/acquisition plan with 88 representative assets and contact/occlusion/hands/rear/from-below/difficult/multi-person coverage
promotion boundary:              MF-P2-09.09 remains NEEDS KEVIN because the required >=30 approved-gold benchmark and correction-minute deltas do not exist; no promotion certificate was minted
verification:                    28 focused integration tests, 9 pose registry/governance tests, and the full 774-test suite passed; Ruff clean; tracker validation clean
tracker:                         MF-P2-09.01 through .08 complete with evidence; .09 blocked; project now tracks 402 items
evidence:                        qa/live_verification/civitai_auxiliary_integration_20260713.json; qa/fixtures/civitai_pose/stress_plan.json
...
## 2026-07-13 - Specialist-aware S11 committee integration completed

**Result:** PASS for machine-side integration; real-gold VLM calibration remains correctly blocked.

raw specialist evidence:            S11 now validates S06 auxiliary authority, geometry, strict binary paths, detector/checkpoint identity, class/confidence/box metadata, and supplies the evidence to local Qwen plus every exact-image-eligible cloud teacher
collision protection:               auxiliary protected-only proposals join every local, cloud, and tournament candidate protected-neighbor check
tournament participation:           exact ontology specialist masks are separately registered as proposal-only candidates with honest checkpoint provenance and complete-map hard QA; one checkpoint remains one independent source
missing-label recovery:              if S09 has no pixels for a specialist-supported label, S11 preserves the true empty S09 baseline, exposes the raw candidate as 100% disagreement, and still runs review/collision/tournament routing instead of silently dropping the part
disagreement routing:               specialist/final union disagreement >=.03 emits AUX-S11-001, forces careful routing, pins evidence, and remains independent of both the final mask and reviewer opinion
live evidence:                      the real img_2ca794d19be9 auxiliary package exposed hair plus both-hand/both-foot candidates and 59,843 protected pixels; a hair review measured .526939 disagreement
controller proof:                   qwen2.5vl:7b returned pass on that live hair evidence in 45.270 s, while deterministic component-count QA correctly forced fail, proving model confidence cannot clear hard evidence
local runtime:                      qwen2.5vl:7b primary, llava:13b fallback, and qwen2.5:7b-instruct text model all live-smoked and were digest-registered; the unloadable mllama fallback was removed
cloud governance:                   vlm/cloud-teacher enablement is reconciled; actual transmission remains default-deny and requires exact SHA-256, clear-adult/rights evidence, named-provider approval, credential, and budget; only one image is currently eligible
calibration boundary:               production_gate.json remains absent and approved_gold_count=0; exactly 20 distinct frozen QA-passing human-approved gold packages are required, so no synthetic or machine-approved substitute was created
verification:                       40 focused tests and the full 825-test suite passed; Ruff clean; tracker rebuild/validate/report clean
tracker:                            MF-P4-07.01 through .03 complete; MF-P4-07.04 blocked NEEDS KEVIN; MF-P4-05.01/.04 remain honestly blocked; project now tracks 406 items
evidence:                           qa/live_verification/s11_auxiliary_committee_integration_20260713.json; qa/live_verification/s11_specialist_live_probe_20260713/result.json
...
## 2026-07-14 - Ontology-v2 documentation, tracker, and generated references aligned

**Result:** PASS for checklist-I documentation alignment; production remains body_parts_v1.

core doc audit:                     all 18 documents 00-17 scanned; version-sensitive authority aligned in docs 00, 01, 02, 04, 11, 12, and 17; no unqualified 57-class conflict remains
SOP/schema authority:               doc 11 names the separate v2 pilot, non-transferable review authority, 65-state gate, and visible-anatomy addendum; doc 04 names manifest_v2 schema, review, ambiguity, hash, and QA contracts
dataset/multi-person authority:     doc 12 matches ontology-specific immutable DVC publication; doc 17 requires one complete 56-v1 or 65-v2 ontology per instance and forbids image-level version mixing
generated reference:               `Plan/OntologyV2/VERSIONED_REFERENCE.md` is derived from both generated ontologies; `tools/generate_ontology_version_reference.py --check` proves the exact unchanged 56-row prefix and IDs 56-64 additions
tracker repair:                     item master and tracker now agree on 414 items; P0-P8, D1-D11, G1-G9, phase-file, schema examples, and live-state wording are current; rebuild kept 414/414 with zero orphaned and validation clean
D1/CLI help:                        D1 consistently states 56 active-v1 PART drafts and 65 only after gated v2 activation; root, draft, dataset, and restore help surfaces expose the same authority
verification:                       47 focused documentation/schema/CLI tests and all 865 live-root tests pass; ontology/schema/reference generators current; Ruff and changed-file Black clean
authority unchanged:               no v2 gold, human pilot, champion, serving activation, or production-default change was claimed
evidence:                           `qa/live_verification/ontology_v2_documentation_alignment_20260714.json`

## 2026-07-14 - Docs 18-22 and SAM 3.1 tracker reconciliation completed

**Result:** PASS for checklist/traceability integrity; implementation and real-evidence gates remain honestly open.

expanded scope:                     414 legacy/core records plus 195 atomic modernization records = 609 live items across 19 parsed phase files; no duplicates or orphans
traceability:                       `Plan/Items/TRACEABILITY_18_22_SAM31.md` maps every requirement group in docs 18-22 and the approved SAM 3.1 handoff; doc 18 contributes exactly 70 one-to-one items
ontology status migration:          55 of the 60 previously checked ontology entries retained completion because current evidence satisfies the strengthened verify clauses; five were not carried forward where the obsolete age predicate or missing real activation/holdout evidence violates the new contract
truth authority:                    human-anchor train/calibration/holdout, autonomous-certified, weighted pseudo-label, and machine-candidate counts/gates remain separate; certified P5/D5 counts exclude pseudo-labels and calibration/holdout anchors
governance/lifecycle:               active schema v2, unified schema+policy validation, per-provider content/license decisions, planned/installed/benchmarked/promoted states, transactional promotion/rollback, hard-bucket non-inferiority, SAM 3.1 installation/benchmark/rollback, and recurring 90-day currency review all have atomic tracker coverage
autonomy:                           selective certificates, independent-family tournaments, deterministic vetoes, bounded repair, mixed random+risk audits, revocation, residual routing, tier-weighted training, and autonomous multi-person certification have explicit verification/blocker clauses
owner policy reconciliation:        no age-eligibility gate is active tracker authority; rights, provenance, lane-specific content compatibility, provider terms, credentials, spending approval, and human-anchor audit authority remain independently enforced
known implementation mismatches:    MF-P2-10.02, MF-P4-09.01, and MF-P4-10.03 are failed because current drafting, QA, and cloud-eligibility code/tests still carry the obsolete age predicate; no false completion was recorded
derived schema repair:              regenerated inactive `manifest_v2.schema.json` from its truth-tier/autonomous-certification generator after the full suite exposed deterministic drift
verification:                       tracker rebuild parsed 609/609 with zero orphans; validate clean; traceability tests 5 pass; focused modernization suite 164 pass; governance/truth/autonomy/CVAT suite 92 pass; derived-schema focused suite 47 pass; full repository suite 894 pass; Ruff check and format clean
live result:                        450/609 complete (73.9%); the lower percentage is the honest fully expanded system denominator, not legacy/core-only progress

## 2026-07-14 - Autonomous-certified truth reached package, dataset, and provider contracts

**Result:** PASS for the implemented contract slice; real SAM 3.1/checkpoint benchmarks, human-anchor calibration volume, and live certificates remain open.

truth schemas:                      manifest v1/v2 now preserve four explicit tiers, anchor partitions, configured loss weights, and certificate-bundle provenance; generated manifest_v2 drift check is current
partition isolation:                new anchors require train/calibration/holdout; S14 enforces image-level and pHash<=6 duplicate-group isolation; autonomous/pseudo examples are train-only and machine candidates are excluded
dataset authority:                  schema-2 sample_weights and build manifests report all tier counts, certified training count, and effective weight units; only anchor-train plus autonomous-certified satisfy volume/coverage gates
autonomous finalization:            separate certificate-driven S13 path reruns hard QA, validates every covered label/risk bucket/fingerprint, binds evidence/final-mask hashes, clears human attribution, freezes immutable autonomous truth, and never writes human_approved_gold
calibration:                        certificates are schema 2 pooled-risk-bucket artifacts anchored to human_anchor_gold, with covered labels/contexts, exact pipeline fingerprint, Wilson/exact upper bounds, expiry, and hash binding; legacy certificates are non-authoritative by default
SQLite/reindex:                     schema v2 adds a per-package truth table; rebuilds preserve tier, partition, weight, and certificate-bundle hash without rewriting the legacy workflow state machine
provider architecture:             provider-neutral role protocols and family-level independence accounting landed; config keeps incumbents active and SAM3.1/RF-DETR/RTMW/RTMO/SAM3D/BiRefNet/Qwen3-VL/EoMT challengers shadow-only with explicit rollback
verification:                       focused modernization compatibility 135/135 passed; full repository suite 899/899 passed in 165.08 s; post-suite pHash/dataset/autonomy 37/37 and training/dataset/compiler 28/28 passed; Ruff check/format clean on changed files
tracker:                            rebuild parsed 609/609 with zero orphans and validation clean; MF-P1-13.02 complete; MF-P1-13.07, MF-P2-11.12, and MF-P5-10.01 honestly partial pending their remaining explicit verify clauses
not claimed:                        no new provider installed/promoted, no live SAM3.1 benchmark, no CVAT parallel upgrade, no real certificate population, no dataset-count increment, and no zero-touch/accuracy performance claim

## 2026-07-14 - Partition capabilities and certified training gates became executable
...
volume authority:                   launcher and tracker both require 200 certified training packages for P5; D5 requires 300 plus >=80% of coverage cells at 8 and every required attribute at 40; pseudo effective weight cannot satisfy either threshold
active learning:                    weekly retrain artifacts now name certified_training_package_count and new_certified_plus_50 rather than collapsing machine-certified truth into approved human gold
verification:                       full repository suite passed 914/914 after all changes; focused drafting/cloud/provider suite passed 51/51; partition/dataset/autonomy/launcher suites passed; Ruff clean
tracker:                            MF-P1-13.03, MF-P5-10.01, MF-P5-10.02, and MF-P5-10.05 complete with specific evidence
not claimed:                        no certified package count changed, no real P5/D5 gate passed, no training run executed, and no challenger/provider was promoted

## 2026-07-14 - Checkpoint license precedence and lifecycle-aware doctor reporting landed

**Result:** PASS for governance/doctor implementation; live doctor is blocked only by disk headroom.

license precedence:                 optional layered reviews bind repository and checkpoint sources, timestamped snapshot hashes, checkpoint effective scope, and a canonical evidence-bundle hash; a checkpoint prohibition overrides repository permission
tamper behavior:                    conflicting allowed/prohibited evidence fails activation, changing the checkpoint decision without rehashing fails, and an explicitly allowed rehashed checkpoint passes
CI gate:                            the dedicated registry bundle covers schema, legacy bypass, duplicate keys, license/content decisions, lifecycle, checkpoint precedence, hard-bucket promotion, and rollback; local mirror passed 29/29
lifecycle doctor:                    planned challengers are reported but not treated as missing mandatory installs; promoted provider activation governance and every registered promoted file-backed smoke remain mandatory
live inventory:                     planned=11, installed=4, benchmarked=0, promoted=10, reference_only=2, retired=0; 14 registered file-backed smokes passed (installed=4, promoted=10)
live doctor:                        PASS=10 WARN=0 SKIP=0 FAIL=1; torch/cu128, registered models, CVAT 2.24.0, project, SAM2 Nuclio, Ollama image, WSL round-trip, PNG, SQLite, and GPU lock passed
current blocker:                    disk_free failed at 7.6 GiB; new ingest/install/training must remain blocked until governed disk headroom is restored
verification:                       governance/schema/registry/provider CI bundle 29/29; doctor/model/governance focused suite 35/35; Ruff clean
not claimed:                        doctor all-green, any optional challenger installation, benchmark, promotion, or disk cleanup

## 2026-07-14 - Provider contracts and autonomous finalization rollback closed

**Result:** PASS for provider interfaces, legacy SAM2 compatibility, and atomic autonomous package finalization.

provider contracts:                 version 1.0 runtime-checkable identities and strict box/mask evidence cover person/concept detection, interactive segmentation, geometry, pose, silhouette, and VLM review
conformance:                        callable incumbent/challenger adapters exercise all seven roles; both SAM2 incumbent and a fake challenger pass the same contract suite; malformed geometry/masks fail
legacy provenance:                  historical SAM2 identifiers load through aliases without rewriting the source manifest; emitted canonical metadata preserves original alias and binds the historical document SHA-256
atomic certification:               DVC callback completion is followed by evidence and canonical final-mask-set hash rechecks; DVC exception, evidence drift, or post-QA mask drift restores the exact pre-certification package
verification:                       provider suite 6/6; package atomic/QC suite 19/19; full repository suite passed 926/926; repository-wide Ruff passed
tracker:                            MF-P2-11.01, MF-P2-11.02, and MF-P1-13.07 complete; rebuild/validation remain structurally clean
not claimed:                        runtime pipeline migration to every provider adapter, any challenger benchmark/promotion, or a live autonomous certificate population

## 2026-07-14 - Autonomous repair semantic boundary and cloud retry controls closed

**Result:** PASS for implementation and regression safety; Task 23 remains non-gold and unresolved toes remain correctly routed to human review.

semantic regression:               Task 23 v6 is explicitly invalidated because a whole-foot candidate impersonated `right_foot_base` and all four reviewers passed it despite visible toes
specialist authority:              whole-hand/whole-foot outputs are now parent-union visual/localization evidence only; they cannot enter an atomic tournament
atomic derivation:                 bare-foot support produces exact disjoint `foot_base` and `toes` candidates only through a provenance-recorded COCO WholeBody heel/big-toe/small-toe MTP split
deterministic vetoes:              pose-backed guards reject whole-foot-as-base, heel-as-toes, and multi-fingertip-as-hand-base semantics before reviewer acceptance; closed-shoe cases remain exempt from the bare-foot split
review evidence:                   local/cloud prompts now include ontology side, parent, area, components, boundary code, and human-readable boundary text; parent-union contours are explicitly marked non-atomic
failure behavior:                  local VLM transport failures become confidence-0 uncertain votes; no timeout or unavailable reviewer can crash the job or become pass
schema recovery:                   the one local retry now receives the exact contract violation instead of a generic JSON reminder; validation remains strict and a second invalid response remains confidence-0 uncertain
cloud controls:                    diagnosis and convergence share 9-call/job and 6-call/label caps; duplicate pre-convergence diagnosis is disabled; definite unbilled HTTP rejections release reservations; unknown post-dispatch usage remains pessimistically charged
live replay:                       v8 left `right_foot_base` and `right_toes` in `residual_human_queue`; base ended local pass .98 without required cloud votes, toes ended local uncertain 0.0; neither label converged
saved-evidence probe:              after schema-specific retry feedback landed, local Qwen re-audited the exact saved v8 `right_toes` candidate as pass 1.0 in 138.245 s; the probe made zero cloud calls and zero CVAT writes
CVAT authority:                    no Task 23 annotation or review package was changed; the existing human-edited task remains protected from autonomous publication
verification:                       focused integration suite passed; final full repository suite passed 926/926 in 206.2 s; Ruff check and changed-file format check clean; tracker validate/report clean
tracker:                            MF-P4-08.07 evidence refreshed; MF-P4-08.08 remains blocked NEEDS KEVIN for >=30 reviewed human anchors, paired correction-time measurements, and any authorized paid calls
evidence:                           `qa/live_verification/autonomous_repair_execution_20260714.json`; `qa/live_verification/cvat_task23_autonomous_repair_v6_20260714/SEMANTIC_INVALIDATION.json`; `qa/live_verification/cvat_task23_autonomous_repair_v8_20260714/live_result.json`

## 2026-07-14 - Offline provider selection and shadow-only lifecycle gate closed

**Result:** PASS for provider selection and evaluation-only shadow wiring; no optional provider installation or benchmark is claimed.

offline fallback:                    `concept_detector.offline_fallback` is pinned to the registered local GroundingDINO checkpoint with `execution: local` and `billing: none`; hosted-only or paid substitutions fail closed
active authority:                    active role owners resolve through `provider_catalog` into the authoritative external/model registry and must have lifecycle `promoted`; planned and merely installed negative fixtures are rejected
honest reviewer state:               installed Qwen2.5-VL remains an executable shadow/rollback candidate but no longer falsely owns an active provider role before lifecycle promotion; Qwen3-VL variants remain planned and skipped
shadow execution:                    the provider-neutral runner gives every runnable challenger the identical ordered sample set, records planned challengers explicitly as skipped, and emits a hash-bound `evaluation_only_no_runtime_or_promotion_authority` manifest
GroundingDINO role:                  because SAM 3.1 is not installed/promoted, concept discovery has no promoted primary; the reproducible local GroundingDINO path remains available only through the explicit fallback boundary and existing S06 implementation
verification:                        provider/config/governance/model-registry suite passed 42/42; Ruff check passed on the changed implementation and tests
tracker:                             MF-P2-11.11 and hard blocker MF-P2-11.12 complete; rebuild parsed 609/609 with zero orphans, validation clean, unresolved hard blockers reduced to 18
not claimed:                         SAM 3.1/Qwen3-VL/RF-DETR/pose/geometry/BiRefNet variant installation, live challenger execution, benchmark win, lifecycle promotion, or active-role switch

## 2026-07-14 - Autonomy throughput, quality, and confidence metrics separated

**Result:** PASS for the metric schema and dashboard boundary; no quality or certification result is inferred from the zero-touch target.

throughput:                          `zero_touch_fraction` has the exact numerator `zero_touch_packages`, denominator `eligible_packages`, and display label `Zero-touch throughput`
blinded quality:                     mask IoU and boundary F1 require a separately counted blinded `human_anchor_gold` holdout population
statistical confidence:              audited failure outcomes produce separate one-sided 95% Wilson aggregate and conservative exact zero-serious-failure upper bounds
schema behavior:                     the report schema rejects extra `accuracy`, `confidence`, or `quality` fields under throughput and rejects any conflated zero-touch label
configuration:                       autonomy config loading requires the exact three-domain reporting contract and fails on label/field drift
verification:                        focused autonomy suite passed 30/30; full repository suite passed 936/936; repository-wide Ruff passed
tracker:                             MF-P4-11.01 complete; rebuild parsed 609/609, zero orphans, validation clean, 18 unresolved hard blockers
not claimed:                         95% zero touch achieved, measured mask accuracy, a live failure bound, a populated certification stratum, or a production autonomy promotion

## 2026-07-14 - Anatomy-v2 seeded QC fixtures became one-defect/one-check exact

**Result:** PASS for all eleven active anatomy-v2 QA checks.

fixture isolation:                   QC-V2-001..010 and QC-V2-012 each have one seeded defect whose complete failed-check list contains exactly that QC; coupled part-map, atomic-mask, and derived-union invariants remain valid where they are not the target
good fixture:                        the canonical visible-surface case passes all eleven checks
verification:                        focused anatomy-v2 QA suite passed 14/14; MF-P4-09.02 complete; tracker rebuild/validation clean

## 2026-07-14 - New provider discovery became planned-only and append-only

**Result:** PASS for discovery/import governance; no live provider was discovered or installed by this implementation test.

import boundary:                     `maskfactory external import-discovery` accepts one exact, hash-bound discovery record and can emit only lifecycle `planned`, unresolved-license, content-unclear, evaluation-only provider metadata
active authority:                    pipeline provider roles and the model registry remain byte-for-byte unchanged; discovery input cannot carry lifecycle/active fields and duplicate provider keys are rejected
historical evidence:                 import records bind prior/updated registry hashes, active-role digest, model benchmark-certificate digest, and the previous record hash in an append-only JSONL chain
registry preservation:               existing provider mappings and hand-maintained YAML remain unchanged except for insertion at the dedicated planned-challenger boundary
verification:                        focused discovery/governance/provider suite passed 24/24; MF-P7-07.02 complete; tracker rebuild/validation clean
not claimed:                         a live currency discovery, artifact download, license decision, installation, benchmark, promotion, or active-provider replacement

## 2026-07-14 - Mode-B interactive serving became role/lifecycle resolved

**Result:** PASS for provider-neutral interactive serving and legacy compatibility.

runtime selection:                   production runtime resolves `interactive_segmenter` from the pipeline role catalog and authoritative lifecycle registries before lazily loading any GPU provider
contract path:                       `ContractInteractiveRefiner` accepts the versioned provider contract, validates identity, click/prompt geometry, proposal geometry, positive anchoring, and deterministic candidate ranking
swap evidence:                       a fake promoted contract provider serves the same refine surface without SAM-family branching
legacy compatibility:               historical `sam2.1_hiera_large` resolves to canonical `sam2_1_large`; the proven SAM2 implementation remains only the explicit incumbent loader/fallback
provenance wording:                  new responses use provider-neutral `interactive_segmenter_refine`; immutable historical SAM2 evidence remains unchanged and its regression test still passes
verification:                        focused serving/provider/live-evidence suite passed 34/34; MF-P6-06.01 complete; tracker rebuild/validation clean
not claimed:                         SAM 3.1 serving, any active-role promotion, or a new live latency measurement

## 2026-07-14 - API and ComfyUI serving provenance became truth/lifecycle explicit
...
direct test predictors:              injected/unregistered predictors are labeled lifecycle `unregistered` with unclear content and ineligible/missing license/certificate/rollback evidence rather than impersonating a promoted provider
verification:                        focused registry/serving suite passed 38/38; full repository suite passed 941/941; repository-wide Ruff passed
tracker:                             MF-P6-06.02 complete; 609/609 rebuild, zero orphans, validation clean, 18 unresolved hard blockers
not claimed:                         an autonomy certificate, certified serving output, benchmark certificate for legacy champions, rollback declaration, or active provider promotion

## 2026-07-14 - Certificate-aware serving versus residual/audit routing closed

**Result:** PASS for routing metadata and negative authority fixtures; no live certificate population is claimed.

current verification:                serving rechecks lifecycle schema, certificate hash/scope/expiry, exact pipeline fingerprint, and revocation state instead of trusting a historical accepted flag
certified route:                      only a currently covered, unrevoked `calibrated_auto_accepted` lifecycle emits effective tier `autonomous_certified_gold` and destination `served_without_routine_review`
audit route:                          preselected random/risk audits are withheld to `cvat_preselected_audit` while retaining their historical certified tier
residual route:                       stale fingerprint, tampered certificate, revoked scope, or non-certified lifecycle emits effective tier `machine_candidate`, exact residual reason, and `cvat_residual_review`
historical truth:                     `historical_truth_tier` remains explicit when current serving eligibility is downgraded; every route has `authoritative_human_gold: false`
verification:                        serving-route schema plus focused autonomy/serving suite passed 50/50; MF-P6-06.04 complete; tracker rebuild/validation clean
not claimed:                         a populated live certificate stratum, actual zero-routine-review serving volume, or a CVAT audit publication

## 2026-07-14 - Ontology-v2 failure acquisition mapping verified exhaustively

**Result:** PASS for deterministic boundary/side/clothing failure acquisition actions.

closed vocabulary:                   all nine canonical v2 reasons map to exactly one category and unique action with governed review states, views, and occlusion contexts
acquisition authority:               every action routes to `hard_case_holdout`, requires governed adult-source/human review authority, and forbids fabricated, hidden, unreviewed, projected, or amodal positives
failure queue:                       aliases and unknown reasons fail closed; acquisition-plan rendering consumes the same policy instead of maintaining a second mapping
verification:                        every mapping is called twice and must return byte-equivalent structured output; focused operations/failure suite passed 14/14
tracker:                             MF-P7-06.02 complete; rebuild/validation clean

## 2026-07-14 - SAM 3.1/autonomous-gold modernization continuation verification

**Result:** PASS for all code/schema/tracker work in this continuation; live doctor remains correctly blocked by disk headroom only.

completed tracker items:             MF-P2-11.11, MF-P2-11.12, MF-P4-11.01, MF-P4-09.02, MF-P7-07.02, MF-P6-06.01, MF-P6-06.02, MF-P6-06.04, and MF-P7-06.02
full verification:                   all 944 collected repository tests passed; repository-wide Ruff passed
live doctor:                         PASS=10 WARN=0 SKIP=0 FAIL=1; CUDA/PyTorch, all 14 registered model smokes, CVAT API/project, SAM2 Nuclio, Ollama vision, WSL round-trip, strict PNG, SQLite, and GPU lock passed
remaining doctor blocker:            `disk_free` failed at 8.1 GiB; new ingest/provider installation/training remains blocked until a larger governed drive or safely approved storage cleanup restores required headroom
tracker integrity:                   609/609 parsed, zero orphans, structural validation clean, 18 unresolved hard blockers
not claimed:                         full modernization completion, doctor all-green, optional provider installation/benchmark/promotion, CVAT upgrade/rollback, live autonomous certificate volume, or headline demonstrations

## 2026-07-14 - Autonomous risk buckets and empirical pooling became fail-closed

**Result:** PASS for MF-P4-11.08; no live calibration population or production certificate is claimed.

versioned policy:                    `configs/autonomy_risk_buckets.yaml` and its bundled schema define exactly large parts, small parts, hands/feet, hair/boundaries, clothing/materials, sensitive anatomy, occlusion/contact, multi-person overlap, and out-of-distribution buckets with explicit worst-risk precedence
empirical exchangeability:           pairwise predeclared false-accept/serious-failure rate deltas and Wilson-interval overlap are recomputed from exact audit records; the report is canonical-hash bound to the policy, records, strata, comparisons, decision, and timestamp
sparse/nonexchangeable behavior:     any under-floor stratum abstains; any measured nonexchangeable pair splits; neither is allowed to borrow pooled confidence
certificate authority:               cross-label/context certification now requires an explicit registered risk bucket plus recomputable passing exchangeability evidence; certificates bind both the canonical risk policy and pooling-evidence hashes
negative fixtures:                   missing pooling evidence, sparse strata, nonexchangeable strata, malformed features, unregistered buckets, and tampered evidence all fail closed
verification:                        focused autonomy/schema tests passed 48/48; full repository suite passed 961/961; repository-wide Ruff and changed-file Ruff formatting passed
tracker:                             MF-P4-11.08 complete; rebuild parsed 609/609, zero orphans, validation clean, 18 unresolved hard blockers
not claimed:                         populated human-anchor calibration strata, empirical production exchangeability, a passing live autonomy certificate, or measured autonomous quality

## 2026-07-14 - Signed technology-currency review became operational and fail-closed

**Result:** PARTIAL for MF-P7-07.01 and MF-P7-07.03; the machinery and live signed review exist, but the current review correctly fails missing incumbent evidence.
...
seeded evidence:                     signature/payload tamper, input drift, expiry, artifact hash, content decision, benchmark missing/stale, rollback missing, and forged-pass fixtures each fail closed
verification:                        full repository suite passed 972/972 at this slice; repository-wide Ruff and changed-file formatting passed
tracker:                             MF-P7-07.01 partially complete 70%; MF-P7-07.03 partially complete 80%; neither is claimed complete until the live review passes and CI can enforce it

## 2026-07-14 - Weekly mixed-audit operation and serious-failure drill passed

**Result:** PASS for MF-P7-07.04.

registered cadence:                  Windows Task Scheduler job `MaskFactory_WeeklyQaMining` is enabled for Mondays at 10:00 local and invokes the exact versioned `tools/weekly_qa.ps1`
runtime repair:                      replaced unavailable PowerShell-5.1 `ISOWeek`, repaired `bash -lc` quoting, and introduced a WSL `python` shim to the governed Windows Python 3.11 while preserving the required WSL/PYTHONPATH execution boundary
scheduled evidence:                 an observed Running-to-Ready launch completed with `LastTaskResult=0`; next scheduled run remains 2026-07-20 10:00 local
weekly output:                       the current run emitted `qa/autonomy/audit_queues/2026-W29.json`; population zero is reported honestly as an empty queue while the unbiased-random plus risk-oversample selection contract remains intact
serious-failure drill:              `qa/autonomy/serious_failure_drills/5be01a9b7ac736c7b015761c/report.json` passed after five exact serious false accepts, immediately revoked the fingerprint scope, removed serving/certified-training eligibility, and opened a governed retraining task
Windows atomicity fix:               scoped revocation temp names are now short enough for legacy MAX_PATH while final collision-free fingerprinted marker names remain unchanged
verification:                        full repository suite passed 973/973; repository-wide Ruff, changed-file formatting, PowerShell parsing, WSL-boundary regression, exact script execution, and live scheduled execution passed
tracker:                             MF-P7-07.04 complete; 609/609 rebuild, zero orphans, structural validation clean

## 2026-07-14 - Candidate perturbation stability became a certificate prerequisite

**Result:** PASS for MF-P4-11.07.

frozen suite:                        every candidate stability record covers resize, crop, color, prompt, and horizontal flip exactly once with strict binary masks and explicit inverse-alignment state
side correctness:                    horizontal-flip evidence is inverse-aligned before comparison and the reported label must match the ontology's reciprocal `swap_partner`; unsided labels remain unchanged
risk thresholds:                     all nine versioned buckets have explicit IoU, boundary-F, and area-delta thresholds; out-of-distribution evidence always abstains even when pixel-identical
certificate boundary:               every explicit pooled-risk certificate now requires passing, hash-valid, policy-current stability evidence for every covered label and binds all stability evidence hashes
operational surface:                 `maskfactory autonomy evaluate-stability` creates the versioned schema-validated evidence consumed by certificate creation
negative fixtures:                   missing perturbations, geometry regression, flip-label mismatch, wrong inverse alignment, OOD, tampering, scope drift, and missing per-label stability evidence fail closed
verification:                        full repository suite passed 986/986; repository-wide Ruff and changed-file formatting passed
tracker:                             MF-P4-11.07 complete; rebuild parsed 609/609, zero orphans, structural validation clean

## 2026-07-14 - Multi-person certification risk strata became separate from solo

**Result:** PASS for MF-P8-11.01.

versioned buckets:                   duo baseline/overlap, small-group baseline/overlap, contact, occlusion, scale disparity, truncation, crowding, and identity ambiguity are separate explicit strata with worst-risk precedence
authority separation:                assignment accepts only `duo` or `small_group`; solo/unknown contexts cannot enter multi-person certification and solo audit rows are rejected before exchangeability evaluation
context integrity:                   duo baseline/overlap evidence cannot populate small-group buckets and small-group evidence cannot populate duo buckets
statistical behavior:                exact-context pooling uses the same predeclared 95% exchangeability machinery, per-stratum sample floor, sparse abstention, and nonexchangeable split behavior
verification:                        full repository suite passed 1001/1001; repository-wide Ruff and changed-file formatting passed
tracker:                             MF-P8-11.01 complete; rebuild parsed 609/609, zero orphans, structural validation clean
## 2026-07-14 17:20 UTC - Autonomous multi-person candidate hard gate completed

**Result:** MF-P8-11.02 is complete with a non-overridable image-level tournament gate.

Implementation:              `src/maskfactory/autonomy/multi_person_gate.py` evaluates the complete promoted-instance candidate before tournament eligibility
Existing hard QA:             QC-035 instance silhouette exclusivity and QC-036 zero cross-person bleed remain BLOCK-severity prerequisites
Identity containment:         AUT-MP-001 requires each promoted instance atomic union to exactly reproduce its own visible silhouette, with zero missing or outside pixels
Relationship reciprocity:     AUT-MP-002 requires exact contact/contact or occludes/occluded_by directional pairs and agreement with recorded instance neighbors
Contact-band reciprocity:     AUT-MP-003 requires a nonempty directional band for every relationship and rejects missing or unexpected bands
Tournament enforcement:       duo/small_group calls without an explicit passing gate hard-veto every candidate; failed checks are preserved in each candidate's veto list
Seeded verification:          QC-035 overlap, QC-036 identity bleed, missing promoted-person coverage, one-way contact, wrong occlusion inverse, and missing bands all block
Focused regression:           37 tests pass across the new gate, legacy QC-035..038, and autonomy tournament/calibration coverage; Ruff lint and format checks pass
## 2026-07-14 17:28 UTC - Exact-scope multi-person certification and abstention completed

**Result:** MF-P8-11.04 is complete; multi-person autonomy now fails closed on scope or evidence uncertainty.

Certificate binding:           schema-2 certificates now bind `instance_context` (`solo`, `duo`, or `small_group`) in their authenticated payload
Exact scope:                   verification requires exact label, covered context, risk bucket, instance context, and pipeline fingerprint
Selective abstention:          OOD, material drift, critic disagreement, sparse/nonexchangeable pooling, identity ambiguity, risk-bucket mismatch, and evidence fingerprint drift all hard-veto the image candidate
Missing authority:             a duo/small-group tournament without both passing geometry evidence and passing scope evidence routes to residual review
Uncertified multi-person case: even a high-scoring QA-clean candidate routes to residual review when its exact current certificate is absent, stale, or differently scoped
Negative verification:         tests cover wrong instance context, risk bucket, fingerprint, absent certificate, OOD, drift, disagreement, sparse evidence, and ambiguous identity
Focused regression:           46 tests pass across multi-person scope/gates, certificate calibration, serving routing, and autonomy operations; Ruff lint/format checks pass
## 2026-07-14 17:36 UTC - Multi-person automatic/residual/audit image-group routing completed

**Result:** MF-P8-11.05 is complete with certificate-aware per-instance routing and one partition per source image.

Automatic route:               exact-current certificate-covered instances serve without routine CVAT review
Residual route:                only invalid/revoked/stale/non-certified instances are listed for `cvat_residual_review`
Preselected audit route:       an image-level audit selection sends every promoted instance to `cvat_preselected_audit` while retaining certified historical authority
Image-group integrity:         one `truth_partition` is assigned to every pN instance; any residual instance makes the complete source image residual for dataset purposes
Runtime enforcement:           `validate_instance_split_integrity` is called on the emitted image-instance partition map
Training/package compatibility: certificate verification now passes risk bucket and instance context through serving, pseudo-dataset, and autonomous package-finalization surfaces
Regression:                    serving, multi-person scope, autonomy lifecycle/pseudo-dataset, autonomous package atomicity, and dataset-builder tests pass; Ruff and format checks pass
## 2026-07-14 17:49 UTC - Mixed multi-person audits and exact-stratum revocation completed

**Result:** MF-P8-11.06 is complete with image-group sampling and immediate exact-stratum authority removal.

Audit selection unit:          complete p0..pN source image; no isolated instance can be selected without its siblings
Sampling:                      deterministic pre-outcome random component plus risk-bucket oversampling, with per-bucket minimums
Serious failure taxonomy:      cross-person bleed, identity, contact, and occlusion are explicit serious outcomes
Revocation scope:              exact risk bucket + duo/small_group context + pipeline fingerprint; unrelated buckets and fingerprints remain valid
Serving enforcement:           a matching marker forces `withheld_for_residual_review` with `certificate_multi_person_stratum_revoked`
Training enforcement:          the certified-training manifest builder omits every lifecycle under a matching revoked stratum
Retraining:                    outcomes emit a governed multi-person retraining task and preserve corrected-gold hashes
Full regression:               1,025 tests collected and passed; repository Ruff lint clean; all newly changed autonomy/routing/test files pass Ruff format checks
Formatting caveat:             repository-wide Ruff format reports pre-existing differences in 16 other dirty Python files; this task did not mechanically rewrite unrelated in-progress changes
## 2026-07-14 17:58 UTC - Autonomous labor/quality/confidence metrics contract implemented

**Result:** MF-P7-07.05 implementation is complete, but the tracker remains partial because real autonomous-run denominators do not exist yet.

Schema version:                autonomy metrics 2.0.0
Throughput:                    zero-touch packages / eligible packages only
Human workload:               human touches per 100 images, routine-touch fraction, audited fraction, residual-review fraction, and changed pixels per 100,000 predicted pixels
Blinded quality:               image-disjoint human-anchor holdout IoU and boundary-F remain a separate domain
Observed failures:             audit false-accept and serious-failure rates use audited packages as their explicit denominator
Statistical confidence:        95% one-sided upper bounds remain separate from observed rates
Fail-closed validation:        missing denominators, denominator drift, population mismatch, and throughput-as-accuracy/confidence/quality labels are rejected
Focused verification:         26 metrics/truth/tracker tests pass; Ruff lint and format checks pass
Live-state honesty:            tracker metric values remain null rather than fabricating measurements before a real autonomous cohort exists

## 2026-07-14 17:42 UTC - Timestamp correction for the preceding two entries

**Result:** The `17:49` and `17:58` headings immediately above were manually projected timestamps entered ahead of the machine clock. Their evidence, order, commands, and results are valid, but those two clock labels are not. This append-only correction is authoritative: both operations completed before this 17:42 UTC correction, and no future-time execution should be inferred.

## 2026-07-14 17:46 UTC - Post-migration full regression passed

**Result:** The complete post-metrics repository suite passed: 1,033 tests collected, 100% executed, exit code 0. Repository-wide Ruff lint passed; the eight files changed in the final multi-person/metrics slice passed Ruff format checks. Tracker validation reports 609 items, zero structural problems, and 16 unresolved hard blockers.

## 2026-07-14 19:25 UTC - Storage modernization and SAM 3.1 source runtime verified

**Result:** The machine modernization is live and the official SAM 3.1 source/runtime identity is frozen; checkpoint activation remains fail-closed behind Kevin's required Meta/Hugging Face gate acceptance.

Model storage:                     `C:\Comfy_UI_Main\models` is a verified junction to `F:\MaskFactory_Offload_20260714\Comfy_UI_Main\models`; 725 files totaling 23.842 GiB reconciled at the target and the largest-file hashes matched
Safe cleanup:                      removed only the unused Docker `ollama` volume (18.47 GB, zero links) and unused `ollama/ollama:latest` image (zero containers); all CVAT/Nuclio artifacts were preserved
WSL relocation:                    Ubuntu-22.04 moved with `wsl --manage --move` to `F:\MaskFactory_Offload_20260714\WSL\Ubuntu-22.04`; the 39.72 GiB VHDX, distro registration, SAM environment, CUDA access, and Docker integration survived the move
Capacity:                          C: recovered above the 200 GiB production gate (251.8 GiB at final doctor); F: retained 315.9 GiB free
Service recovery:                  Docker Desktop restarted and all 21 CVAT/Nuclio containers returned; CVAT API/project and the SAM2 Nuclio interactor passed live probes
SAM 3.1 source:                    official `facebookresearch/sam3` commit `5dd401d1c5c1d5c3eedff06d41b77af824517619`, tree `3ddfc8fc42fe1cac4e7d81d7476401fc497fe66e`, archive SHA256 `6a1e59bc12639e34d4cf613851954af33a1c5409c8aa09b0f47254225414cb16`
SAM 3.1 checkpoint identity:       official `facebook/sam3.1` revision `daa63191845a41281374e725f4c9e51c7a824460`, `sam3.1_multiplex.pt`, 3,502,755,717 bytes, SHA256 `0567debeec80ba4ac6369540c6c248025283cb3ff2b92827509e57e2b3541cb6`; unauthenticated fetch returned HTTP 401
Isolated runtime:                  `/home/kevin/mfenvs/sam31-5dd401d1`, Python 3.12.13, torch 2.10.0+cu128, torchvision 0.25.0+cu128, SAM3 0.1.0, CUDA capability 12.0; image-model and multiplex-video-predictor imports, CUDA allocation, and `pip check` passed
Packaging corrections:            pinned setuptools 80.9.0 and explicit einops 0.8.1, pycocotools 2.0.10, and psutil 7.0.0 to close upstream runtime omissions reproducibly
Activation boundary:              registry lifecycle remains `planned` and `verify_license: true`; no checkpoint inference, latency, VRAM, determinism, benchmark, or promotion is claimed until Kevin accepts the official interactive checkpoint terms
Tracker:                           MF-P0-17.03 complete with immutable source/license/checkpoint/runtime evidence; MF-P0-17.04 remains `NEEDS KEVIN` for the official checkpoint gate, then live inference qualification
Verification:                      first post-move cold torch probe alone exceeded 30 seconds; direct retry passed in 1.733 seconds on the RTX 5060 and the complete warm doctor passed `PASS=11 WARN=0 SKIP=0 FAIL=0`, including all 14 registered model hashes

## 2026-07-14 19:52 UTC - Parallel CVAT v2.69 migration and rollback rehearsal passed

**Result:** MF-P0-17.12 is complete with an independently running, fully migrated CVAT challenger and an untouched production rollback path.

Target selection:                  selected signed stable CVAT v2.69.0, published 2026-06-22, at commit `f6ae1dca443b94dd01288dfc9ac6ffe988218f0c`; did not select v2.70.0 because it was a same-day release observed only hours after publication
Source preservation:               CVAT v2.24.0 at commit `9fafd98f0c0588b775db8f98648569dfa48292b5` remained the active incumbent at `localhost:8080`
Consistent backup:                 stopped the incumbent briefly and archived all seven Docker volumes to `F:\MaskFactory_Offload_20260714\CVAT\backups\cvat-v2.24.0-20260714T1933Z`; every archive has a recorded SHA-256 and the incumbent returned with unchanged counts and healthy SAM2
Parallel isolation:                project `cvat269` uses 18 unique container names, seven prefixed volumes, loopback ports 18080/18090, source checkout on F:, and globally unique Traefik routers; no production volume or container name is reused
Routing correction:                both Traefik instances watch Docker globally, so upstream's generic route names collided; the override now replaces rather than merges labels with `cvat269`-scoped routers
Migration:                         database, task media, keys, logs, and ClickHouse event data restored into new target volumes; Redis/KVrocks volatile caches started clean; Django `migrate --check --noinput` reports zero pending migrations
Database identity:                 source and target canonical SHA-256 both `8c1693fd160674960dab350d3ff472785846c07dc10c6ed1c8c00ce6382ea4d9` across 2 users, 2 projects, 21 tasks, 21 jobs, and 367 labeled shapes
Media identity:                    source and target canonical SHA-256 both `73eaa7204e00b1d11daed83a521c97325ba5a03463ac4f7ee42c26c5c651ee82` across 95 files and 216,949,704 bytes
Authenticated API identity:        both task listings contain the same 21 IDs and canonical SHA-256 `8244eae24fe917a8362add9ed8cc0db00374743ac1c85ff5df62ed41ae3ceb55`
Rollback drill:                    stopping the complete target made only port 18080 unavailable; production about API remained HTTP 200 and production SAM2 remained running/healthy; target then restarted and reported 2.69.0
Authority boundary:                v2.69 remains a parallel challenger; no production promotion, human-gold mutation, task-status mutation, or serverless authority change occurred
Evidence:                          `qa/live_verification/cvat_parallel_upgrade_v269_20260714.json`, `configs/cvat-compose.parallel-v269.yml`, and three isolation/evidence regression tests
Final verification:                expanded full repository suite passed 1,039/1,039; repository-wide Ruff passed; production doctor passed `PASS=11 WARN=0 SKIP=0 FAIL=0`; tracker rebuilt 609/609 with zero structural problems

## 2026-07-14 20:27 UTC - RF-DETR Medium shadow integration and rollback verification passed

**Result:** MF-P2-11.05 is complete; RF-DETR is a live, governed person-detector challenger while YOLO11 remains the active and rollback provider.

Source/runtime freeze:              RF-DETR 1.7.1 commit `6e1620e751f3c814ead8648cada51ceff9029e5c`, tree `71babdc6e97fb6785403d7b69b50cc278897c333`, isolated WSL Python 3.12.13 runtime, torch 2.11.0+cu128, torchvision 0.26.0+cu128, CUDA 12.8, RTX 5060 sm_120
Checkpoint identity:                official Medium COCO weight, 404,992,918 bytes, MD5 `7223f764a87b863f02eb8d52bf0ce2ee`, SHA-256 `749ff6071828aaffac63e204c4f4135ed3d6cdae4d702e086c360edc3b5768c8`; Apache-2.0 code/official non-Plus weight lane
Provider integration:               `RfdetrPersonDetector` conforms structurally to the versioned `PersonDetector`, invokes the exact isolated runtime, emits typed person-only boxes, and fails closed on checkpoint, version, image, threshold, determinism, count, process, or geometry drift
Selection authority:                `rf_detr_medium` is installed/shadow-only; `yolo11m_person` remains both active and rollback; an attempted installed-provider role switch is rejected because active providers require lifecycle `promoted`
Frozen live comparison:             governed adult four-person bus fixture; RF-DETR and YOLO11 each found 4/4 people, all four matched one-to-one at IoU >= 0.5, mean matched IoU `0.97931262`, zero unmatched boxes
Rollback replay:                    YOLO11 output SHA-256 `c646981ad0a4c629c32bba5b955c9824e22ab1d2740000e936e2e4124c115a03` was byte-identical before and after the rejected role switch
Latency boundary:                   YOLO11 comparison ran on CPU and RF-DETR on CUDA, so recorded elapsed values are diagnostic and explicitly not a valid latency benchmark
Evidence:                           `qa/live_verification/rfdetr_medium_runtime_20260714.json`, `qa/live_verification/rfdetr_yolo11_shadow_comparison_20260714.json`, `env/rfdetr_runtime.lock.json`
Verification:                       22 focused provider/runtime/evidence tests passed; full repository suite collected and passed 1,051/1,051; repository-wide Ruff passed; tracker rebuilt 609/609 with zero structural problems

## 2026-07-14 20:54 UTC - Qwen3-VL 4B/8B local shadow reviewers qualified

**Result:** MF-P2-11.09 is complete; both approved Qwen3-VL sizes are exact local shadow reviewers behind `VlmReviewer`, while Qwen2.5-VL and LLaVA remain incumbent/fallback.

Official source:                    `QwenLM/Qwen3-VL` commit `96588727e44c78b25ba03ea03b8e12f7e64fd0da`; official 4B/8B Instruct GGUF revisions `1cd86afb9a95c410a6038ab3b40d8b578c892266` and `f982a07559d4a2f6c8744d840bf6fccab30eea96`; Apache-2.0 license layer SHA-256 `7339fa418c9ad3e8e12e74ad0fd26a9cc4be8703f9c110728a992b193be85cb2`
Exact 4B artifact:                 `qwen3-vl:4b-instruct-q4_K_M`, manifest SHA-256 `ee4b975b58c17ce268cd19d40db35d5edc64603035d2ffc1fee1968eb0947f7b`, model blob SHA-256 `16b83be682148a4d8201dbf720ea7eace5de98b69f63f05e0c908b4d7977ecb5`, 3,295,636,231 bytes
Exact 8B artifact:                 `qwen3-vl:8b-instruct-q4_K_M`, manifest SHA-256 `0533d74300e4f9bc367d675d4e64ffd073d50ff16a2b4096cc2e8a1cf8c96319`, model blob SHA-256 `1329cd5ab37e5ef6faef37cd8ff8660ffc96ed5b47b34fddf8eb0912e6c82466`, 6,140,415,975 bytes
Runtime isolation:                  native local Ollama 0.32.0 outside the project cu128 Python core; loopback-only API; RTX 5060 with 8,151 MiB; inference context fixed to 4,096 for bounded 8 GB operation
Strict provider boundary:           `Qwen3VlmReviewer` verifies exact local digest, qwen3vl family, Q4_K_M quantization, and vision capability, then uses Ollama JSON Schema plus the canonical parser; invalid identity, missing inputs, process failure, or schema drift fails closed
Live 4B performance:                3,527,545,978 bytes VRAM; cold 8.54 s; two warm requests max 1.33 s; cold and warm response hashes matched
Live 8B performance:                5,534,897,929 bytes VRAM; cold 12.42 s; two warm requests max 1.99 s; the cold warm-up verdict differed, but both post-warm-up responses were exact; the lock therefore claims post-warm-up determinism only
Fallback/authority:                 Qwen2.5-VL incumbent digest `5ced39dfa4ba...` remained installed and passed doctor image JSON; LLaVA remains configured fallback; a missing Qwen3 model returns HTTP 404; challengers cannot author masks, approve gold, or clear BLOCKs
Evidence:                           `env/qwen3_vl_ollama.lock.json`, `qa/live_verification/qwen3_vl_ollama_runtime_20260714.json`, and live provider-adapter responses for both exact variants
Verification:                       focused Qwen/provider/governance suite passed 38 tests; complete repository suite passed 1,058/1,058; repo-wide Ruff passed; doctor `PASS=11 WARN=0 SKIP=0 FAIL=0` with provider lifecycle planned=11/installed=6/promoted=10 and 238.0 GiB free
Tracker:                            MF-P2-11.09 complete; MF-P0-17.13 increased honestly to 50%; MF-P4-10.12 is 10% because the human-anchor 40-panel/200-case benchmark and promotion certificate remain outstanding

## 2026-07-14 21:16 UTC - BiRefNet Dynamic/HR/HR-matting shadow integration passed

**Result:** MF-P2-11.08 is complete; all three official BiRefNet variants are installed behind the versioned `SilhouetteProvider`, while BiRefNet-general remains active and rollback authority.

Official snapshots:                 Dynamic revision `280306042f57b7a33854319da62fd86aaa89ec4c`, HR revision `a7a562f6fd16021180f2f4348f4de003a2d3d1e1`, and HR-matting revision `5d6b6f8adcb5b417c871b1d84ceaae9871355b7f`; each immutable checkpoint is 444,473,596 bytes with a distinct frozen SHA-256
License/content:                    official MIT source/license snapshot is frozen; both governed adult lanes are explicitly allowed for every variant under the private-local profile
Installation:                       exact official source/config/handler/checkpoint files installed to the F-backed `models/bv/dyn`, `models/bv/hr`, and `models/bv/hrm` paths with per-file hashes
Preprocessing correction:           Dynamic uses native dimensions padded to stride 32; HR and HR-matting were requalified using the official 2048-square contract rather than the earlier invalid small-crop input
2048 memory result:                 HR and HR-matting completed deterministically, but each reported 10,133,235,200 bytes peak allocated and 15,334,375,424 bytes peak reserved, exceeding the 8,589,410,304-byte dedicated-GPU budget
Governed 8 GB path:                 HR and HR-matting therefore default to the official 1024 evaluation mode; 2048 remains an explicit high-memory shadow option and cannot be promoted on this machine without an approved alternate runtime
Provider contract:                  adapter validates variant, revision, checkpoint, image, person box, resolution, two-run determinism, NPY artifact hash, confidence payload hash, shape, range, and strict-mask nondegeneracy before returning typed output
Live strict outputs:                adult four-person bus fixture, left-person context; Dynamic/HR/HR-matting each returned a full-canvas 1080x810 boolean mask with proposal confidences 0.9881/0.9914/0.9795
Live matting output:                HR-matting returned full-canvas float32 alpha in 0..1 with fractional-alpha fraction `0.0638626`; alpha payload SHA-256 `96cbb5a4f74d1704d1ddfdb43a3eedd32f091d566ae48c48d737210404ebc194`
Fallback/authority:                 injected challenger failure returns an incumbent-authored proposal with the incumbent identity; pipeline active and rollback both remain `birefnet_general`; no challenger promotion or gold authority is claimed
Evidence:                           `env/birefnet_variants.lock.json`, installation/runtime/provider-integration live reports under `qa/live_verification/`, governed setup/smoke/runner/integration scripts, and fail-closed contract tests
Verification:                       focused provider/runtime/governance suite passed 36/36; complete repository suite collected and passed 1,074/1,074; repository-wide Ruff passed; doctor `PASS=11 WARN=0 SKIP=0 FAIL=0` with provider lifecycle planned=8/installed=9/promoted=10 and 234.7 GiB free
Tracker boundary:                   MF-P2-11.08 complete; MF-P3-08.03 remains partial because the frozen human-anchor role benchmark, boundary/leakage comparison, and promotion certificate do not yet exist

## 2026-07-14 21:33 UTC - Specialist candidate provenance and family deduplication enforced

**Result:** MF-P3-08.07 is complete; specialist outputs remain distinct tournament candidates while correlated checkpoints can contribute only one independent-family vote.

Typed provenance:                   `MaskCandidateInput` accepts exact `ProviderIdentity` records and emits candidate evidence with provider keys and deduplicated model families
Candidate preservation:             BiRefNet Dynamic and HR evidence remain separate candidate IDs and separate provider records; no family deduplication removes or overwrites either output
Independence accounting:            the same Dynamic/HR pair contributes one `birefnet` family; adding RF-DETR contributes a second independent family, not a third correlated vote
Composite candidates:               family credit is derived from typed provider identities; a declared family set that differs from provider identities fails before tournament evidence is built
Tournament guard:                   an inflated `independent_sources` integer that differs from the provenance model-family set raises `AutonomyTournamentError` before scoring
Legacy boundary:                    historical evidence without typed provider provenance remains readable for compatibility but cannot be emitted as a modern specialist provenance summary
Verification:                       39 focused autonomy/provider/specialist tests passed; complete repository suite collected and passed 1,078/1,078; repository-wide Ruff passed

## 2026-07-15 00:11 UTC - RTMW-X and RTMO governed shadow pose integration passed

**Result:** MF-P2-11.06 is complete; both modern pose challengers are live behind `PoseProvider`, while DWPose remains active/rollback and MediaPipe remains the independent hand vote.

Immutable sources:                  signed MMPose v1.3.2 commit `5408bc76f5b848cf925a0d1857899011d8c5b497`, tree `592d7336c9dd65a3f19f96c8bbcf0956bcf97426`; rtmlib v0.0.15 reference commit `f1bac7d80c88305534dbcdde12da39f6db4c3953`, tree `aa0ac704492788c82c37bc323ba98fff45edbf26`; clean Apache-2.0 license snapshots frozen
RTMW-X checkpoint:                 official 384x288 cocktail14 model, 369,720,404 bytes, SHA-256 `f840f2044fe46cb3821b7cea86be83e1f6cba406ccd28f5475ac010412dcda95`; exact config SHA-256 `3317ddf7b9ad9d8046422254b8549276e25c07170e02e5d1ccad09c0cf8a3623`
RTMO checkpoint:                   official RTMO-L body7 CrowdPose 640x640 model, 178,452,521 bytes, SHA-256 `5bafdc11e43fba1a834e1323013108831b3e1e0761681dbe7a37896a179f2183`; exact config SHA-256 `0360c8a3e085f1a8958833df44310b7e132946cf400f51834f48387e5b7eb957`
Runtime isolation:                 existing local CUDA Python plus isolated `models/runtime_cache/rtm_pose_deps` layer with NumPy 1.26.4, mmcv-lite 2.1.0, MMEngine 0.10.7, MMDetection 3.2.0, and packaged MMPose `.mim` metadata; base Comfy environment unchanged; runtime fingerprint `0e5374ea...`
PyTorch compatibility:             exact checkpoint hashes are verified before a process-local `weights_only=False` compatibility wrapper is enabled for the two trusted OpenMMLab files; unrelated EDPose compiled-op import is isolated without modifying the signed source tree
Exact vocabulary:                  RTMW-X exposes COCO-WholeBody-133; RTMO exposes the official CrowdPose-14 vocabulary including `top_head`; all left/right labels are declared and tested as character-anatomical sides
Crowd ownership:                   RTMO performs global multi-person inference, selects the requested person by maximum bbox IoU, exposes every candidate box, and emits a stable instance key; distinct requested character boxes select distinct candidates in regression tests
Live qualification:                governed adult bus fixture; RTMW-X deterministic 1x133 output; RTMO deterministic seven-candidate 7x14 output; live requested-person IoU `0.99999946`; combined smoke peak reserved memory remained at or below 717,225,984 bytes
Fallback/authority:                process, provenance, shape, hash, vocabulary, determinism, ownership, or confidence drift fails closed; an explicit failure fallback preserves the DWPose provider identity; both challengers remain installed/shadow-only and cannot author gold
Evidence:                           `env/rtm_pose.lock.json`, `qa/live_verification/rtm_pose_install_20260714.json`, `qa/live_verification/rtm_pose_runtime_20260714.json`, `qa/live_verification/rtm_pose_provider_20260714.json`, setup/smoke/runner/integration scripts, and focused provider/runtime-lock tests
WSL recovery:                       a cold Ubuntu VHD read fault corrupted Docker's integration proxy; Docker Desktop was cleanly restarted, the proxy restored as a 23,873,528-byte executable, both CVAT deployments/volumes survived, and Ubuntu memory/filesystem/mount checks passed
Legacy smoke repair:                `faceparse_bisenet` moved from the degraded WSL VHD to the same torch 2.11/cu128 local runtime using exact source commit `d2e684c`; it reproduced the registered output SHA-256 `8c3235e1...` exactly in 9 seconds
Verification:                       focused RTM/provider/runtime/governance tests passed; complete repository suite collected and passed 1,087/1,087; repo-wide Ruff passed; final doctor `PASS=11 WARN=0 SKIP=0 FAIL=0`, including all 14 exact registered model-smoke hashes and provider lifecycle planned=6/installed=11/promoted=10

## 2026-07-15 00:29 UTC - SAM 3D Body source frozen at human gate; EoMT-DINOv3 challenger installed

**Result:** MF-P2-11.10 is complete; MF-P2-11.07 is honestly blocked at 20% on Meta's human checkpoint terms; aggregate isolated-runtime coverage MF-P0-17.13 is now 80%.

SAM 3D Body source:                official repository commit `b5c765a0d89d789985e186d396315e7590887b94`, tree `7b5a2a23619659a679d64f096201c6504cbebe30`, clean local source snapshot, SAM License file SHA-256 `315341214a...`
SAM 3D Body checkpoint identity:    official gated DINOv3 repository revision `11aaa346c7204874a1cbafe3d39a979080b2c55a`; public license SHA-256 `b3a5a0e2...`; `model.ckpt`, `model_config.yaml`, and `assets/mhr_model.pt` each return HTTP 401 `GatedRepo`
SAM 3D Body blocker:               `NEEDS KEVIN`: accept Meta's SAM License/contact-sharing terms at the official Hugging Face repository; DensePose remains active/rollback and no source-only inference or authority claim was made
EoMT selection:                    original-author `tue-mps/eomt-dinov3-coco-panoptic-small-640`, immutable revision `602edaa2839daf6cb3de3ad46c176098c3be9090`, MIT, selected for 8 GB reliability
EoMT checkpoint:                   93,450,628-byte safetensors SHA-256 `1fed3231445cce739e368c1828f49215459ca33ba56b6712d48e3058274c5d6f`; exact config SHA-256 `8baa79f9...`; preprocessor SHA-256 `ce155401...`
Live EoMT smoke:                   adult bus fixture, two-run deterministic 1080x810 panoptic output with eight segments; payload SHA-256 `8ff0753481...`; peak reserved 700,448,768 bytes; warm inference 0.039196 seconds
Ontology boundary:                 pretrained COCO-panoptic-133 vocabulary is evidence only and cannot author MaskFactory masks; training contract discards that head and creates a random 65-class `body_parts_v2` head bound to vocabulary SHA-256 `7f78e0c340...` and ignore index 255
Baseline/authority:                SegFormer-B3 and Mask2Former-Swin-B remain explicit baselines; EoMT is installed for training/shadow evaluation only, with no active provider, rollback role, gold authority, or promotion claim
Evidence:                           `env/eomt_dinov3_runtime.lock.json`, `configs/training/eomt_dinov3_small_v2.yaml`, `qa/live_verification/eomt_dinov3_runtime_20260715.json`, `qa/live_verification/sam3d_body_source_gate_20260715.json`, and fail-closed contract tests
Verification:                       complete repository suite collected and passed 1,095/1,095; repository-wide Ruff passed; tracker rebuilt 609/609 with zero structural problems

## 2026-07-15 00:57 UTC - WSL/Docker recovery revalidated; SAM2.1 continuity gate hardened

**Result:** The Docker Desktop/Ubuntu integration proxy failure is cleared and MF-P6-06.03 is 70% complete with live incumbent/fallback evidence; no SAM3.1 promotion or displaced-incumbent rollback is claimed.

WSL/Docker state:                   Ubuntu-22.04 and docker-desktop run under WSL2; Ubuntu-22.04 is restored as the default distro with user `kevin` and home `/home/kevin`, systemd reports running, the Docker integration proxy is executable mode 755 at 23,873,528 bytes, and Docker Engine 29.4.3 responds
Service preservation:              all production CVAT 2.24, parallel CVAT 2.69, and Nuclio/SAM2 containers survived; production CVAT reports 2.24.0 and the isolated Host-routed challenger reports 2.69.0
Root cause boundary:                the reported permission-denied proxy was transient Docker/WSL integration corruption after an unhealthy VHD session; the recovered executable proxy and healthy distro round-trip disprove an ongoing `/etc/wsl.conf` or home-directory permission fault
Live machine verification:          project doctor passed `PASS=11 WARN=0 SKIP=0 FAIL=0`; all 14 file-backed model smoke hashes matched; CVAT API/project, SAM2 Nuclio, Ollama image input, WSL round-trip, CUDA, SQLite, strict PNG, disk, and GPU lock checks passed
Selection hardening:                governed provider selection now resolves and validates explicit rollback and local non-billable OOM fallback bindings, including authoritative lifecycle state; planned artifacts cannot become recovery providers
SAM2.1 continuity:                  `sam2_1_large` remains promoted/active/rollback, `sam2_1_base_plus` remains installed as local OOM fallback, and both exact checkpoint SHA-256 values resolve through the registry
Fail-closed SAM3.1 probe:           an attempted `sam3_1` active-role edit is rejected at lifecycle `planned`; selection and production pipeline bytes remain exact before/after the probe
Live CVAT refinement:               `pth-sam2` answered the governed synthetic task with a valid 256x256 binary mask containing 21,491 foreground pixels
Evidence:                           `qa/live_verification/sam2_incumbent_rollback_20260715.json`, `tools/verify_sam2_incumbent_rollback.py`, expanded provider-selection regression tests
Verification:                       93 focused provider/serving/production tests passed; complete repository suite passed 1,098/1,098; repository-wide Ruff passed; tracker validation reports zero structural problems
Tracker boundary:                   MF-P6-06.03 is partially complete at 70%; the remaining post-promotion SAM3.1-to-SAM2.1 one-command rollback rehearsal is blocked by the real MF-P2-11.15 frozen benchmark/certificate gate

## 2026-07-15 01:13 UTC - Specialist non-inferiority margins frozen before results

**Result:** MF-P3-08.08 is 85% complete with a hash-locked pre-result policy; incorporation into the still-unfrozen human-anchor provider matrix remains pending under MF-P2-11.13.

Frozen policy:                     `specialist_noninferiority_v1`, frozen at 2026-07-15 01:00 UTC, SHA-256 `6b5a939f9aeeda0025ef39b93b308a9c7ed2ef2caaff1cc76d05a6acba4d5b55`
Role coverage:                     nine distinct policies cover hand/finger, foot/toe, chest/pelvic, hair/matting, clothing/accessory, whole silhouette, pose, geometry, and repeated-instance roles
Expanded contract:                 406 exact label/context/zero-regression buckets; every row uses the frozen convention that positive delta is challenger improvement and must carry the exact predeclared margin
Source binding:                    hashes bind both active ontologies, the QA hard-class list, anatomy-v2 canonical vocabulary, single-person autonomy risk policy, and every multi-person risk stratum
Regression boundary:               cross-person bleed, side/identity/front-back, protected-region, hard-QA, determinism, OOM/crash, and rollback reliability are explicit zero-regression metrics rather than averageable scores
Immutability:                      manifest self-hash plus a code/CI locked hash rejects a threshold edit even if the editor recomputes the self-hash; result artifacts must bind the frozen hash and postdate it
Completeness:                      result validation requires every expanded bucket exactly once, rejects omitted/duplicate/extra buckets and margin drift, and derives pass from observed delta rather than trusting a claimed flag
Negative proof:                    a primary aggregate win with one chest/pelvic hard-bucket result below its frozen margin was rejected deterministically
Authority boundary:                the manifest declares thresholds only; it contains no human-anchor results and grants no benchmark, promotion, serving, or gold authority
Evidence:                           `qa/governance/benchmark_matrices/specialist_margins_v1.json`, `qa/live_verification/specialist_benchmark_margins_20260715.json`, bundled schema, validator, verifier, and regression tests
Verification:                       30 focused policy/registry/currency tests passed; complete repository suite passed 1,105/1,105; repository-wide Ruff passed

## 2026-07-15 01:25 UTC - Currency-review CI integrity and decision binding activated

**Result:** MF-P7-07.03 is 90% complete; CI now rejects stale, tampered, or input-drifted signed reviews and runs the seeded gate suite, while strict review-pass enforcement remains honestly blocked by MF-P7-07.01.

Review refresh:                    immutable review `de453d50e6cbf92e05be3b7a`, SHA-256 `baa7f8031c721263f15ba3d9f8e4b01ab51c155ef6608beffad61dabd98af938`, covers five active roles and validates signature, schema, age, event, exact roles, and all current input hashes
Decision binding:                  the review dependency set now includes `Plan/DECISIONS_LOG.md`; changing the signed decision log produces `active_input_hash_mismatch`, proven by a dedicated seeded regression
History preservation:              superseded signed reviews remain immutable under `qa/governance/currency/reviews/`, and the new review chains the prior review SHA-256
Runtime repair:                    the declared `cryptography==49.0.0` dependency was installed into the project virtual environment, so the shipped `maskfactory` command performs Ed25519 verification directly
CI enforcement:                    the workflow verifies the live review's signature, age, event, active-role recomputation, decision/dependency hashes, registries, pipeline, and rollback-evidence input; a named 12-test currency suite seeds tamper, stale review, changed active input, changed decisions, missing artifact hash, unresolved content, missing/stale benchmark, missing rollback, and forged-pass failures
Strict boundary:                   the current review remains `fail`; strict verification exits 1 only for `benchmark_certificate_missing`, `rollback_evidence_missing`, `rollback_provider_not_distinct`, and the aggregate `currency_review_not_passing`
No weakened gate:                  `--allow-failed-review` is used only for integrity/current-input CI while the signed review is openly failed; CI cannot silently treat the review as passing, and strict enforcement will replace this boundary after MF-P7-07.01 gains real benchmark certificates and rollback drills
Evidence:                           `qa/live_verification/currency_ci_gate_20260715.json`, signed current review plus immutable history, workflow, CLI dependency binding, and seeded tests
Verification:                       focused currency suite passed 12/12; CI YAML parsed; live integrity command passed; strict negative command failed with exact expected codes; complete repository suite passed 1,106/1,106; repository-wide Ruff passed

## 2026-07-15 01:42 UTC - Cross-runtime isolation matrix frozen and verified

**Result:** MF-P0-17.13 is 90% complete; every currently accessible modern runtime is exact-lock/evidence bound and live-qualified, while the two Meta-gated families remain explicit source-only entries rather than inferred successes.

Core preservation:                 the active core remains PyTorch 2.11.0+cu128 on CUDA 12.8; its conda, Python, source-build, and live SAM2 continuity evidence hashes are frozen in the matrix
Qualified isolation:               Qwen3-VL runs behind native Ollama; RF-DETR runs in its WSL venv; EoMT-DINOv3 runs in the external Comfy CUDA runtime; RTMW-X/RTMO use an isolated target layer over that CUDA runtime; each has exact lock and live evidence hashes
Human gates:                       SAM 3.1 and SAM 3D Body are the only `source_only_human_gate` rows; both carry precise `NEEDS KEVIN:` terms-acceptance reasons, checkpoint-inference `not_run` state, and no gold authority
Matrix integrity:                  seven provider families and 15 artifacts are covered; canonical manifest SHA-256 `3fb4e9d2fac03b8ebccabf4a34b062eec1d4060b3e982fad7d5db1283dbef877` is locked in code and CI
Fail-closed checks:                missing/changed artifacts, provider coverage drift, empty isolation boundaries, status overclaims, unexpected human gates, qualified runtimes without lock/evidence, and any gold-authority claim are rejected
Observed drift recovery:           an accidental fresh Qwen smoke changed the evidence bytes; the first matrix verification rejected the stale hash, after which the live report and Qwen lock were deliberately rebound before the matrix was frozen
Live doctor:                       `PASS=11 WARN=0 SKIP=0 FAIL=0`; CUDA core passed, all 14 registered model smoke hashes matched, CVAT 2.24 and SAM2 Nuclio passed, Ollama image JSON passed, WSL round-trip passed, and 215.0 GiB remained free
Evidence:                           `env/provider_runtime_matrix.json`, `qa/live_verification/provider_runtime_matrix_20260715.json`, verifier module/tool, four negative/positive tests, and named CI gate
Verification:                       runtime matrix verifier passed; focused tests passed 4/4; complete repository suite passed 1,110/1,110; repository-wide Ruff passed; signed currency-review integrity remained current
Remaining boundary:                final 10% requires official checkpoint access and live isolated qualification for SAM 3.1 and SAM 3D Body; no gated checkpoint result is claimed

## 2026-07-15 02:09 UTC - Repository normalization and governance hashes reconciled

**Result:** Mandatory pre-commit formatting completed without semantic changes; every affected lock, frozen policy, signed review, test, and live matrix is current again.

Pre-commit result:                 end-of-file, trailing-whitespace, YAML, large-file, Ruff, and Black hooks all pass; the parallel CVAT compose file is explicitly exempted from generic PyYAML because Docker Compose's valid `!override` tag requires Compose-aware parsing
Formatter scope:                   Black normalized 39 Python files; no runtime behavior, threshold, authority, model selection, or benchmark result was changed
Runtime rebinding:                 BiRefNet, EoMT-DINOv3, and RTM reproduction hashes were updated to the normalized source/test bytes; their focused lock suite passed 29/29
Policy rebinding:                  autonomy risk/stability config hashes were updated after EOF normalization; specialist policy retains all nine roles and 406 identical buckets and is administratively refrozen at `605f79e0d4f8354a7a4d445a0a5725af829cd78b85e2e36f91b065576553a739` before any real benchmark result
...
Ext4 confirmation:                   Docker's 04:45 UTC retry failed creating `/root/.docker/desktop` with `input/output error`, confirming the mounted Ubuntu filesystem was not writable rather than merely blocked by Unix permissions
Safe stop:                           `docker desktop stop` completed without elevation; final process inventory is zero Docker Desktop and zero WSL/WSL-host/vmmem processes, so the physical Seagate device may be reseated while Docker remains closed
...
Closed vocabulary:                   56 fixed axes with 445 values, 8 versioned-registry axes, 6 continuous axes, and 18 mandatory high-risk intersections are disjoint, ordered, versioned, and fail closed on unknown or missing values
Source binding:                       exact hashes and value crosswalks bind canonical MaskFactory coverage and the 64 foreground ontology labels plus DAZ character, appearance, solo-pose, scene-formation, duo-relationship, render-pass, and asset-registry policies; any source or crosswalk drift rejects
...
- Tier: **`HARD_QA_PASS_BOUNDED`** for the sampled draft instance corpus
- Not gold; deterministic QC/hash battery only

### Visual QA (real panels, pixel review)
Reviewed `all_parts` / part panels across 9 instance packages. Defects include binary noise/leak, under-segmentation, garment-biased chest, torso fragmentation, multi-person half-fill, exclusivity bleed.
- Highest visual tier: **`VISUAL_QA_REVIEWED_WITH_DEFECTS`**
- **Not** `VISUAL_QA_PASS_BOUNDED` (no clean panels)

### Autonomous repair / abstention
- Evaluated largest-CC noise cleanup on `img_51945db358cb/p0` `left_forearm` binary tile: 20 components, 291 removable noise px
- **`ABSTAIN_BOUNDED`**: not applied to live package (would not clear remaining visual defects; preserve HARD_QA hashes)

### Docker / runtime (re-probed)
- Docker `29.4.3`; CVAT `localhost:8080` ? **2.24.0**; Nuclio `pth-sam2` healthy; Ollama `0.32.1`
- Doctor all-green still **`RUNTIME_BLOCKED`** (~10.8 GiB disk; WSL preflight latency)

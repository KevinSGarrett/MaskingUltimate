...
Ultimate Masking System build-out: all 827 action items from
...
827 items after the docs 18–25/SAM 3.1/DAZ/bridge reconciliation). Rerun it any time `Plan\Items\*.md` is
...
```
Statuses can be comma-separated. No filters = list everything (827 lines).
...
```
Confirms 827 non-orphaned items, no duplicate/invalid statuses, validates the

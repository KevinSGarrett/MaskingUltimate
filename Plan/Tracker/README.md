...
Ultimate Masking System build-out: all 906 action items from
...
906 items after the docs 18–28/SAM 3.1/DAZ/bridge/adult-corpus reconciliation). Rerun it any time `Plan\Items\*.md` is
...
```
Statuses can be comma-separated. No filters = list everything (906 lines).
...
```
Confirms 906 non-orphaned items, no duplicate/invalid statuses, validates the

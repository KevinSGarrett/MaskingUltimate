...
def run_text(args, env=None, check=True) -> str:
    return run(args, env=env, check=check).stdout.decode("utf-8", errors="replace")
...
def stage_path(env: dict, rel: str, content: str | None = None, mode: str = "100644") -> None:
    if content is None:
        ...
        blob = run(
            ["git", "hash-object", "-w", "--stdin"], env=env, input_bytes=data
        ).stdout.decode().strip()
    else:
...
def main() -> None:
    ...
    for attempt in range(1, 16):
        ...
        head_ops = run_text(["git", "show", f"{head}:Plan/OPS_LOG.md"]).replace("\r\n", "\n")
        if MARKER in head_ops:
            print(f"OPS_LOG already sealed on {head}; nothing to do")
            return

        head_tracker = json.loads(
            run_text(["git", "show", f"{head}:Plan/Tracker/tracker.json"])
        )
        ...
        p1_text = render_p1(tracker).replace("\r\n", "\n")
        ...
        tree = run_text(["git", "write-tree"], env=env).strip()
        commit = run_text(

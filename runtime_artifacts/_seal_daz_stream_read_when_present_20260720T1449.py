...
def main() -> int:
    ...
    GOLD_SLICE.write_text(
        json.dumps(gold_slice, indent=2, sort_keys=True) + "\n", encoding="utf-8"
    )
    if not (
    ...
    document = {
        ...
        "f_daz_read_when_present": {
            ...
            "prior_f_restored_seal": PRIOR_F_RESTORED.relative_to(REPO).as_posix()
            if PRIOR_F_RESTORED.is_file()
            else None,
            "prior_f_restored_seal_sha256": _file_sha(PRIOR_F_RESTORED)
            if PRIOR_F_RESTORED.is_file()
            else None,
            "note": (

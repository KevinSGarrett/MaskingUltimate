...
PRIOR_BLOCK = (
    REPO / "qa" / "live_verification" / "docker_relocation_f_absent_blocked_20260720T1435Z.json"
)
...
def main() -> int:
    ...
    document = {
        ...
        "prior_f_absent_block": (
            PRIOR_BLOCK.relative_to(REPO).as_posix() if PRIOR_BLOCK.is_file() else None
        ),
        ...
        "offload_tree": {
            ...
            "wsl_ubuntu2204_vhdx_gib": (
                round(F_VHDX.stat().st_size / (1024**3), 2) if F_VHDX.is_file() else None
            ),
        },
        "data_health": {
            ...
            "f_datarelocated_sqlite_images": _sqlite_images(F_DATARELOCATED / "maskfactory.sqlite"),
    ...
    print(
        "data_junction ->", junction_target, "pkgs", document["data_health"]["active_package_count"]
    )
    print(
        "wsl_wake exit",
        document["wsl_wake"].get("exit_code"),
        "write_ok",
        document["wsl_wake"].get("write_ok"),
    )
    return 0

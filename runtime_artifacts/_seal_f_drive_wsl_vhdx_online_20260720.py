...
for _attempt in range(3):
    wsl_true_rc, wsl_true_out = run(
        ["wsl", "-d", "Ubuntu-22.04", "--", "/bin/true"], timeout=60
    )
    final_probe_attempts.append({"attempt": _attempt + 1, "exit_code": wsl_true_rc, "output": wsl_true_out})
    if wsl_true_rc == 0:
        break
...
evidence = {
    ...
    "concurrent_sibling_contention_addendum_20260720T1455_1459Z": {
        ...
        "probes": [
            {"at": "~14:55Z", "cmd": "wsl -d Ubuntu-22.04 -- /bin/true", "result": "FAILED: Wsl/Service/CreateInstance/MountDisk/HCS/ERROR_SHARING_VIOLATION (disk in use by another process)"},
            {"at": "~14:57Z", "cmd": "wsl -d Ubuntu-22.04 -- /bin/true", "result": "FAILED: Wsl/Service/CreateInstance/E_FAIL (error code 6, failure step 2)"},
            {"at": "~14:59Z", "cmd": "wsl --shutdown; wsl -d Ubuntu-22.04 -- /bin/true", "result": "PASS: exit code 0 (clean boot, no error)"},
        ],

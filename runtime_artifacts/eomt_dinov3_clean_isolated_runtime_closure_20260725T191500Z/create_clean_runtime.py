...
OUT = ROOT / "runtime_artifacts/eomt_dinov3_clean_isolated_runtime_closure_20260725T191500Z"
BINDING = OUT / "clean_runtime_binding.json"
RECEIPT = OUT / "CLEAN_ISOLATED_RUNTIME_RECEIPT.json"
ENV = OUT / "venv"
SYSTEM_PYTHON = Path("/usr/bin/python3")
LOCK = ROOT / "env/eomt_dinov3_clean_runtime.requirements_v2.lock.txt"
...
def wheel_rows(receipt_path: Path) -> list[dict]:
    return json.loads(receipt_path.read_text())["wheels"]
...
def main() -> int:
    ...
    try:
        ...
        receipt["install"] = run([
            ...
            "--find-links", str(TRANSFORMERS_ROOT / "wheels"), "--find-links", str(TORCH_ROOT / "wheels"), "-r", str(LOCK),
        ])
        ...
        receipt["import_smoke"] = run([
            str(python), "-c",
            "import json,torch,transformers,tokenizers,safetensors,huggingface_hub; from transformers import EomtDinov3ForUniversalSegmentation; print(json.dumps({'torch':torch.__version__,'transformers':transformers.__version__,'tokenizers':tokenizers.__version__,'safetensors':safetensors.__version__,'huggingface_hub':huggingface_hub.__version__,'eomt_class_imported':EomtDinov3ForUniversalSegmentation.__name__},sort_keys=True))",
        ])

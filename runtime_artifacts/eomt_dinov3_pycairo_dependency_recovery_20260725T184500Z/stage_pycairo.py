...
OUT = ROOT / "runtime_artifacts/eomt_dinov3_pycairo_dependency_recovery_20260725T184500Z"
HELPER = ROOT / ".train_cu128_ada/bin/python"
BINDING = OUT / "pycairo_stage_binding.json"
RECEIPT = OUT / "PYCAIRO_STAGE_RECEIPT.json"
WHEELS = OUT / "wheels"
...
def main() -> int:
    receipt = {
        "artifact_type": "eomt_dinov3_pycairo_dependency_recovery_receipt.v1",
    ...
    try:
        ...
        receipt["download"] = run([str(HELPER), "-m", "pip", "download", "--disable-pip-version-check", "--only-binary=:all:", "--no-deps", "--dest", str(WHEELS), "pycairo==1.29.0"])
        if receipt["download"]["returncode"]:
            raise RuntimeError("wheel-only pycairo staging failed")
        files = sorted(path for path in WHEELS.iterdir() if path.is_file())
        if len(files) != 1 or not files[0].name.startswith("pycairo-") or files[0].suffix != ".whl":
            raise RuntimeError("expected exactly one pycairo wheel")
        wheel = files[0]
        receipt["wheel"] = {"filename": wheel.name, "bytes": wheel.stat().st_size, "sha256": sha(wheel)}
        receipt["status"] = "PYCAIRO_ARTIFACT_STAGE_PASS"
        receipt["next_safe_action"] = "Create a new requirements lock that binds this wheel plus the preserved 27-wheel Transformers closure; then run one new isolated-runtime binding."
        ...
        receipt["status"] = "PYCAIRO_ARTIFACT_STAGE_FAIL"

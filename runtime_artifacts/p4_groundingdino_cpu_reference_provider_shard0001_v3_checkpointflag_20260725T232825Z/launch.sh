#!/usr/bin/env bash
set -uo pipefail
ROOT='/workspace/maskfactory/runtime_artifacts/p4_groundingdino_cpu_reference_provider_shard0001_v3_checkpointflag_20260725T232825Z'
...
r={'schema_version':'maskfactory.p4_groundingdino_cpu_provider_receipt.v1','state':'completed' if rc==0 else 'failed','exit_code':rc,'authority_ceiling':'proposal_boxes_only','binding_self_sha256':b['self_sha256'],'report_sha256':sha(os.path.join(root,'groundingdino_report.json')),'checkpoint_sha256':sha(os.path.join(root,'groundingdino_checkpoint.json')),'stderr_sha256':sha(os.path.join(root,'groundingdino.stderr.log')),'claims':b['claims']}

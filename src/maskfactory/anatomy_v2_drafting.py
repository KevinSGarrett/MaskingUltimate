...
approves gold.  Its outputs become eligible for human review only after the
confirmed-adult and visible-pixel gates below pass.
"""
...
def load_anatomy_v2_config(path: Path | str = DEFAULT_CONFIG) -> dict[str, Any]:
    ...
    if not isinstance(governance, dict) or governance.get("allowed_intake_decisions") != [
        "clear_adult"
    ]:
        raise AnatomyV2DraftError("anatomy-v2 drafting must require clear_adult intake")
...
def require_clear_adult(intake_decision: str) -> None:
    if intake_decision != "clear_adult":
        raise AnatomyV2DraftError(
            f"anatomy-v2 drafting requires clear_adult intake, got {intake_decision!r}"
        )
...
def build_anatomy_crop_proposals(
    *,
    ...
    require_clear_adult(intake_decision)
...
def proposal_to_sam2_plan(
    ...
    *,
    ...
    require_clear_adult(proposal.adult_intake_decision)
    ...
    if state in EXPLICIT_NON_CANDIDATE_STATES:
        return AnatomyDraftCandidate(
            ...
            correction,
            {
                ...
                "sam2_required": True,
                "adult_intake_decision": proposal.adult_intake_decision,
            },
        )
    ...
    *,
    ...
    for name in NEW_LABELS:
        ...
        if candidate is None or candidate.mask is None:
            ...
            continue
        if candidate.label != name or candidate.authority != "sam2_refined_non_gold_review_draft":
            raise AnatomyV2DraftError(
                f"anatomy-v2 fusion refuses non-SAM2 candidate authority: {name}"
            )
        ...
        if (
            candidate.provenance.get("adult_intake_decision") != "clear_adult"
            ...
            raise AnatomyV2DraftError(
                f"anatomy-v2 fusion refuses unproven adult/SAM2 candidate provenance: {name}"
            )

...
The checks in this module implement Document 18 QC-V2-001..012 without
...
def load_anatomy_v2_qa_config(path: Path | str = DEFAULT_CONFIG) -> dict[str, Any]:
    ...
    if not isinstance(governance, dict) or governance != {
        ...
        "required_adult_intake_decision": "clear_adult",
        "ignore_index": 255,
...
def run_anatomy_v2_qc(
    ...
    *,
    ...
    return (
        ...
        _qc011(inputs, atomics, set(config["governance"]["permitted_source_origins"])),
...
def write_anatomy_v2_qa_report(
    ...
    *,
    ...
    if tuple(result.qc_id for result in results) != QC_IDS:
        raise AnatomyV2QaError("anatomy-v2 QA report requires exact QC-V2-001..012 results")
...
def _qc011(inputs, atomics, permitted_origins):
    positives = sorted(name for name in NEW_LABELS if atomics[name].any())
    source = inputs.manifest.get("source")
    origin = source.get("source_origin") if isinstance(source, Mapping) else None
    failures = []
    if positives and inputs.adult_intake_decision != "clear_adult":
        failures.append(f"adult_intake_decision={inputs.adult_intake_decision}")
    if positives and origin not in permitted_origins:
        failures.append(f"source_origin={origin}")
    for name in positives:
        if inputs.label_provenance.get(name, {}).get("adult_intake_decision") != "clear_adult":
            failures.append(f"{name}:missing_clear_adult_provenance")
    return QcResult(
        "QC-V2-011",
        "adult_governance",
        not failures,
        f"positive={positives}, violations={failures}",
    )

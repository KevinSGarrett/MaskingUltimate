...
def load_operational_policy(
    ...
    if document["schema_version"] != "1.0.0" or document["policy_id"] != (
        "operational-mask-qa-v1"
    ):
...
def _strict_binary(path: Path) -> np.ndarray:
    ...
    if array.ndim != 2 or array.dtype != np.uint8 or not set(np.unique(array)).issubset(
        {0, 255}
    ):
...
def _evaluate_synthetic_truth_cases(
    ...
    for kind in policy["required_synthetic_case_kinds"]:
        ...
        semantics_passed = {
            ...
            "boundary_shift": (
                not pixel_equal and label_equal and candidate_area == truth_area
            ),
            "missing_area": (
                not pixel_equal and label_equal and candidate_area < truth_area
            ),
    ...
    for binding in report["gate_bindings"]:
        ...
        if not isinstance(gate, dict):
            raise OperationalPolicyError(
                f"certificate QA gate is missing: {binding['gate_id']}"
            )
        gate.update(
...
def validate_operational_policy_report_binding(
    ...
    *,
    ...
    if (
        report["policy"]["policy_id"] != qa_evidence.get("qa_policy_id")
        or report["policy"]["policy_sha256"] != qa_evidence.get("qa_policy_sha256")
    ):

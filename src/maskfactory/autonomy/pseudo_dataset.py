...
from .calibration import AUTONOMOUS_GOLD_CERTIFICATE_SCHEMA, verify_autonomy_certificate
    ...
    *,
    ...
    for path in sorted(lifecycle_root.rglob("*.json")):
        ...
        valid, reason = verify_autonomy_certificate(
            ...
            risk_bucket=risk_bucket,
            pipeline_fingerprint=document["pipeline_fingerprint"],
            allow_autonomous_profile=(
                certificate.get("schema_version") == AUTONOMOUS_GOLD_CERTIFICATE_SCHEMA
            ),
        )
        if not valid:
            continue

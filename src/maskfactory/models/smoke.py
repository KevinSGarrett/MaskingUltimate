...
def birefnet_general_cuda_local(checkpoint: Path, image: Path) -> dict[str, Any]:
    """Run pinned BiRefNet via ComfyUI local CUDA (copy-not-symlink; no WSL)."""
    command = [
        "C:/Comfy_UI_Main/ComfyUI/.venv/Scripts/python.exe",
        str(ROOT / "tools" / "smoke_birefnet_wsl.py"),
        "--checkpoint",
    ...
    try:
        ...
        return {
            "passed": False,
            "output_sha256": "",
            "reason": f"invalid local CUDA smoke output: {exc}: {process.stdout[-1000:]}",
        }


register_smoke_runner("birefnet_general_cuda_local", birefnet_general_cuda_local)
...
def schp_atr_cuda_local(checkpoint: Path, image: Path) -> dict[str, Any]:
    """Run SCHP ATR via ComfyUI local CUDA (Windows paths; no WSL)."""
    env = os.environ.copy()
    env["MASKFACTORY_SCHP_CACHE"] = str(ROOT / "models" / "runtime_cache" / "schp")
    command = [
        "C:/Comfy_UI_Main/ComfyUI/.venv/Scripts/python.exe",
        str(ROOT / "tools" / "smoke_schp_wsl.py"),
        "--checkpoint",
        str(checkpoint.resolve()),
        "--image",
        str(image.resolve()),
    ...
    process = subprocess.run(
        command, capture_output=True, text=True, timeout=900, check=False, env=env
    )
    if process.returncode != 0:
        return {
            "passed": False,
...
register_smoke_runner("schp_atr_cuda_local", schp_atr_cuda_local)

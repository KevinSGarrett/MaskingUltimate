...
class NudeBatchQueue:
    ...
    def checkpoint_box_prompt_masks(
        self,
        *,
        platform: str,
        shard_path: str,
        ...
        """Persist provider-specific draft masks without advancing terminal progress."""
        ...
        for receipt in receipts:
            ...
            if (
                ...
                raise NudeBatchQueueError("box prompt mask sample_index invalid")
            payload = validate_box_prompt_mask_stage_receipt(
                {key: value for key, value in receipt.items() if key != "sample_index"}
            )
            validated.append((sample_index, payload))
        now = time.time()
        ...
        with self._transaction() as connection:
            ...
            for sample_index, payload in validated:
                if sample_index >= int(shard["sample_count"]):
                    raise NudeBatchQueueError("box prompt mask checkpoint exceeds shard")
                payload_sha = _canonical_sha256(payload)
                values = (
                    platform,
                    str(payload["sample_id"]),
                ...
                try:
                    ...
                    if existing is None or existing["payload_sha256"] != payload_sha:
                        raise NudeBatchQueueError("box prompt mask idempotency conflict") from exc
                    retained += 1
            self._event(
                connection,
                platform=platform,
    ...
    def checkpoint_reference_strict_visual_review(
        self,
        *,
        platform: str,
        shard_path: str,
        ...
        """Persist per-record visual votes without terminal promotion or authority."""
        ...
        for receipt in receipts:
            ...
            if (
                ...
                raise NudeBatchQueueError("reference strict visual sample_index invalid")
            payload = validate_reference_strict_visual_stage_receipt(
                {key: value for key, value in receipt.items() if key != "sample_index"}
            )
            validated.append((sample_index, payload))
        now = time.time()
        ...
        with self._transaction() as connection:
            ...
            for sample_index, payload in validated:
                if sample_index >= int(shard["sample_count"]):
                    raise NudeBatchQueueError("reference strict visual checkpoint exceeds shard")
                payload_sha = _canonical_sha256(payload)
                values = (
                    platform,
                    str(payload["sample_id"]),
                ...
                try:
                    ...
                    if existing is None or existing["payload_sha256"] != payload_sha:
                        raise NudeBatchQueueError(
                            "reference strict visual idempotency conflict"
                        ) from exc
                    retained += 1
            self._event(
                connection,
                platform=platform,
                shard_path=shard_path,
                event="reference_strict_visual_checkpoint",
                detail={"inserted": inserted, "retained": retained},
                now=now,
            )
        return {"inserted": inserted, "retained": retained, "terminal_progress_advanced": False}

    def checkpoint_reference_repairs(
        self,
        *,
        platform: str,
        shard_path: str,
        ...
        """Persist bounded per-target repair history without terminal promotion."""
        ...
        for receipt in receipts:
            ...
            if (
                ...
                raise NudeBatchQueueError("reference repair sample_index invalid")
            payload = validate_reference_repair_stage_receipt(
                {key: value for key, value in receipt.items() if key != "sample_index"}
            )
            validated.append((sample_index, payload))
        now = time.time()
        ...
        with self._transaction() as connection:
            ...
            for sample_index, payload in validated:
                if sample_index >= int(shard["sample_count"]):
                    raise NudeBatchQueueError("reference repair checkpoint exceeds shard")
                payload_sha = _canonical_sha256(payload)
                values = (
                    platform,
                    str(payload["sample_id"]),
                ...
                try:
                    ...
                    if existing is None or existing["payload_sha256"] != payload_sha:
                        raise NudeBatchQueueError("reference repair idempotency conflict") from exc
                    retained += 1
            self._event(
                connection,
                platform=platform,
                shard_path=shard_path,
                event="reference_repair_checkpoint",
                detail={"inserted": inserted, "retained": retained},
                now=now,
            )
        return {

...
def v2_supervision_ineligibility(manifest: Mapping[str, Any]) -> tuple[str, ...]:
    ...
    if manifest.get("workflow_status") not in {"approved_gold", "exported"}:
        reasons.append("workflow is not approved_gold/exported")
    qa = manifest.get("qa")
    ...
    for name in sorted(expected_parts & set(parts)):
        ...
        if not isinstance(authority, Mapping) or (
            ...
            or authority.get("source") != "human_review"
            or authority.get("ontology_version") != "body_parts_v2"
            or not authority.get("reviewer")
            or not authority.get("reviewed_at")
        ):
            reasons.append(f"{name} lacks body_parts_v2 human review authority")
        if entry.get("visibility") == "unreviewed_for_v2":
            reasons.append(f"{name} remains unreviewed_for_v2")
        if (
            entry.get("visibility") in V2_VISIBLE_STATES
            and entry.get("status") != "human_approved_gold"
        ):
            reasons.append(f"{name} visible pixels are not human_approved_gold")
    return tuple(sorted(set(reasons)))

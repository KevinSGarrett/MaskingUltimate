...
def certify_autonomous_package(
    package_root: Path,
    *,
    ...
    """Hard-QA, certificate-bind, freeze, and DVC-add autonomous training truth."""
    package_root = Path(package_root)
    if (package_root / ".maskfactory_frozen.json").is_file():
        raise RuntimeError(f"package is already frozen: {package_root}")
    if not 0.5 <= float(training_loss_weight) <= 0.75:
        raise ValueError("autonomous certified training weight must be 0.5..0.75")
    ...
    for certificate in certificates:
        ...
        for label in labels & set(certificate.get("covered_labels", ())):
            ...
            covered.add(label)
    if covered != labels:
        raise ValueError(f"autonomous certificates do not cover labels: {sorted(labels - covered)}")

    dvc_callback = dvc_add or _dvc_add
    if dvc_add is None:
        _require_dvc()
    backup = _snapshot_directory(package_root, "autonomous-certification")
    try:
        ...
        report = _qa_report(package_root, results, timestamp)
        report["consensus"] = {
            "method": "autonomous_certificate_bound_multi_provider",
            "sources": sorted(
                {bucket for certificate in certificates for bucket in certificate["covered_labels"]}
            ),
        }
        report["vlm_review"] = {"model": "certificate_bound", "verdicts": []}
        require_valid_document(report, "qa_report")
        _write_json_atomic(package_root / "qa_report.json", report)
        _write_json_atomic(
            package_root / ".maskfactory_frozen.json",
            {
                "schema_version": "2.0.0",
                "frozen_at": timestamp.isoformat(),
                "authority": "autonomous_certified_gold",
                "pipeline_fingerprint": pipeline_fingerprint,
        ...
        manifest_certification_evidence = json.loads(
            (package_root / "manifest.json").read_text(encoding="utf-8")
        )["certification"]["evidence_sha256"]
        if sha256_file(evidence_path) != manifest_certification_evidence:
            raise RuntimeError(
                "autonomous certification evidence hash drifted during finalization: "
                f"expected {manifest_certification_evidence}"
            )
...
def _stamp_autonomous_manifest(
    package_root: Path,
    *,
    certificates: tuple[dict[str, Any], ...],
    pipeline_fingerprint: str,
    ...
    for entry in manifest.get("parts", {}).values():
        if isinstance(entry, dict) and entry.get("status") != "n/a":
            entry["status"] = "autonomous_certified_gold"
            provenance = entry.get("provenance")
            if isinstance(provenance, dict):
                provenance["human_edit"] = False
            review_authority = entry.get("review_authority")
            if isinstance(review_authority, dict):
                review_authority.update(
                    {
                        "reviewed": True,
                        "reviewer": None,
                        "reviewed_at": timestamp.isoformat(),
                        "source": "autonomous_certification",
                        "ontology_version": manifest.get("mask_ontology_version", "body_parts_v1"),
                    }
                )
    manifest["truth_tier"] = "autonomous_certified_gold"
    manifest["truth_partition"] = "train"
    manifest["training_loss_weight"] = training_loss_weight
        ...
        "certificates": [
            {
                ...
                "covered_labels": sorted(certificate["covered_labels"]),
            }
            for certificate in sorted(
                certificates, key=lambda value: (value["risk_bucket"], value["certificate_id"])
            )
        ],
        "pipeline_fingerprint": pipeline_fingerprint,
    ...
    manifest["workflow_status"] = "autonomous_certified"
    manifest["workflow_updated_at"] = timestamp.isoformat()
    _write_json_atomic(path, manifest)

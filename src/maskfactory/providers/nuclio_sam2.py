    ...
    def get_or_create_image_task(self, *, task_name: str, image_path: Path) -> int:
        ...
        while time.monotonic() < deadline:
            ...
            if status == "finished":
                return task_id
            if status == "failed":
                raise NuclioSam2Error(f"CVAT task data processing failed: {request.get('message')}")
            time.sleep(1)
    ...
    def invoke(
        ...
        *,
        ...
        if expected_shape is not None and mask.shape != expected_shape:
            raise NuclioSam2Error(f"pth-sam2 mask shape {mask.shape} != expected {expected_shape}")

...
WORKFLOW_STATUS_COMPATIBILITY = {
    "autonomous_certified": "approved_gold",
    "weighted_pseudo": "approved_gold",
    "machine_candidate": "in_review",
}
...
class PackageTruthRow:
    ...
    certificate_bundle_sha256: str | None
...
def _manifest_status(manifest: Mapping[str, Any]) -> str:
    ...
    if workflow_status is not None:
        workflow_status = WORKFLOW_STATUS_COMPATIBILITY.get(
            str(workflow_status), str(workflow_status)
        )
...
def expected_package_truth_rows(
    packages_root: Path = DEFAULT_PACKAGES_ROOT,
) -> tuple[PackageTruthRow, ...]:
    """Return explicit per-package truth authority for the SQLite companion index."""
    ...
    for path in sorted(root.rglob("manifest.json")):
        ...
        if tier is None:
            ...
            tier = (
                "human_anchor_gold"
                if manifest.get("workflow_status") in {"approved_gold", "exported"}
                or statuses
                and statuses <= {"human_approved_gold", "human_anchor_gold"}
                else "machine_candidate"
            )
        partition = manifest.get("truth_partition")
        if partition is None and tier == "human_anchor_gold":
            partition = "train"
        weight = manifest.get("training_loss_weight")
        if weight is None:
            weight = {
                "human_anchor_gold": 1.0,
                "autonomous_certified_gold": 0.65,
                "weighted_pseudo_label": 0.20,
                "machine_candidate": 0.0,
            }[str(tier)]
        ...
        if isinstance(certification, dict):
            certificate_hash = hashlib.sha256(
                json.dumps(
                    certification.get("certificates", ()),
                    sort_keys=True,
                    separators=(",", ":"),
                ).encode()
            ).hexdigest()
        rows.append(
            PackageTruthRow(
                ...
                truth_tier=str(tier),
                truth_partition=str(partition) if partition is not None else None,
                training_loss_weight=float(weight),
                certificate_bundle_sha256=certificate_hash,
            )
        )
    return tuple(rows)
...
def reindex_packages(
    *,
    ...
    expected_truth = expected_package_truth_rows(packages_root)
    ...
    with writer_connection(database) as connection:
        ...
        connection.executemany(
            ...
            [tuple(asdict(row).values()) for row in expected_truth],
        )
    return difference

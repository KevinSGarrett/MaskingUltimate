...
def assemble_review_package(
    *,
    ...
    if auxiliary_dir is not None and Path(auxiliary_dir).is_dir():
        ...
        specialist_overlay = auxiliary_dir / "review_overlay.png"
        if specialist_overlay.is_file():
            shutil.copy2(specialist_overlay, overlays / "auxiliary_specialists.png")
        evidence_root = package_root / "annotations" / "auxiliary_specialists"
        for name in ("auxiliary_predictions.json",):
            source = auxiliary_dir / name
            if source.is_file():
                evidence_root.mkdir(parents=True, exist_ok=True)
                shutil.copy2(source, evidence_root / name)
        normalized = auxiliary_dir / "normalized"
        if normalized.is_dir():
            shutil.copytree(normalized, evidence_root / "normalized", dirs_exist_ok=True)

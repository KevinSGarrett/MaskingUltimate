...
from ..providers.civitai_auxiliary import AuxiliaryProviderError, load_material_assists
...
def run_s08_production(
    *,
    ...
    if auxiliary_dir is not None and Path(auxiliary_dir).is_dir():
        try:
            auxiliary_material = load_material_assists(Path(auxiliary_dir), visible.shape)
        except AuxiliaryProviderError as exc:
            raise MaterialError(f"S08 auxiliary material evidence invalid: {exc}") from exc
        unknown = set(auxiliary_material) - set(MATERIAL_IDS)
        if unknown:
            raise MaterialError(f"S08 auxiliary material targets are unknown: {sorted(unknown)}")
        for name, candidate in auxiliary_material.items():
            regions[name] = regions.get(name, np.zeros(visible.shape, dtype=bool)) | (
                candidate & visible
            )
    ...
    for name in (
        auxiliary_material if auxiliary_dir is not None and Path(auxiliary_dir).is_dir() else ()
    ):
        evidence[name] = tuple(sorted(set(evidence.get(name, ())) | {"civitai_auxiliary"}))
    evidence.update(
        {

...
def run_s09_production(
    *,
    ...
    if auxiliary_dir is not None:
        vote_root = Path(auxiliary_dir) / "normalized" / "vote_part_candidate"
        for path in sorted(vote_root.glob("*.png")) if vote_root.is_dir() else ():
            add(path.stem, "auxiliary", np.asarray(Image.open(path).convert("L")))
...
def _validate_weights(weights: Mapping[str, float]) -> None:
    ...
    permitted = (
        required,
        {**required, "custom_bodypart": 0.45},
        {**required, "auxiliary": 0.05},
        {**required, "custom_bodypart": 0.45, "auxiliary": 0.05},
    )
    if not any(

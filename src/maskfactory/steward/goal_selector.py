...
def select_next_plan27_work(
    ...
    *,
    ...
    stalled_item_ids: Iterable[str] = (),
) -> GoalSelection | None:
    """Select the highest-priority unblocked Plan-27 unit.

    Durable active, terminal, ambiguous, superseded, and repeatedly stalled
    identities are supplied by the caller's ledger reconstruction.  When
    inference is unavailable the selector skips only completion units that
    inherently require live inference and continues scanning for useful
    CPU-safe work.
    """
    ...
    exclusions = {
        ...
        "stalled": frozenset(stalled_item_ids),
    }
    overlap = set()

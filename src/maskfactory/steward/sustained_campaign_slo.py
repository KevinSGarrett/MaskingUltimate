...
def _timestamp(value: object, *, field: str) -> datetime:
    ...
    try:
        ...
        raise SustainedCampaignSloError(
...
def _validate_record(
    ...
    *,
    ...
    if (
        ...
        raise SustainedCampaignSloError(
            "campaign_sequence must be a positive integer"
        )
    ...
    validate_campaign_handoff_audit_replay(
        ...
        terminal_adoption_packet_sha256=record[
            "terminal_adoption_packet_sha256"
        ],
    )
...
def evaluate_sustained_campaign_slo(
    ...
    *,
    ...
    if not isinstance(campaign_records, Sequence) or isinstance(
        campaign_records, (str, bytes)
    ):
        raise SustainedCampaignSloError("campaign_records must be a sequence")
    if len(campaign_records) != CAMPAIGN_COUNT:
        raise SustainedCampaignSloError(
            "exactly three mixed campaign records are required"
        )
    records = [
        _validate_record(record, repo_root=Path(repo_root))
    ...
    if sequences != list(range(sequences[0], sequences[0] + CAMPAIGN_COUNT)):
        raise SustainedCampaignSloError(
            "campaign sequence must be ordered and contiguous"
        )
    campaign_ids = [
        record["telemetry"]["campaign_id"] for record in records
    ]
    if len(campaign_ids) != len(set(campaign_ids)):
        raise SustainedCampaignSloError("campaign IDs must be unique")
    if any(
        record["telemetry"]["campaign_kind"] != "mixed" for record in records
    ):
        raise SustainedCampaignSloError(
            "sustained acceptance requires mixed campaigns"
        )
    starts = [
        _timestamp(record["telemetry"]["started_at"], field="started_at")
        for record in records
    ]
    ends = [
        _timestamp(record["telemetry"]["ended_at"], field="ended_at")
        for record in records
    ]
    if any(starts[index] < ends[index - 1] for index in range(1, CAMPAIGN_COUNT)):
        raise SustainedCampaignSloError(
            "campaign times overlap or are not chronological"
        )
    ...
    metrics = {
        ...
        "duplicate_promotions": sum(
            value["duplicate_promotions"] for value in slo_metrics
        ),
        ...
        "authority_bypasses": sum(
            value["authority_bypasses"] for value in slo_metrics
        ),
    }
    gates = {
        ...
        "all_campaign_slos_passed": all(
            record["slo"]["passed"] for record in records
        ),
        "all_handoff_audits_passed": all(
            record["handoff_audit"]["passed"] for record in records
        ),
        "autonomous_preparation_target": (
            metrics["minimum_autonomously_prepared_fraction"] >= 0.8
        ),
        "handoff_suppression_target": (
            metrics["maximum_routine_handoffs_per_campaign_bound"] <= 1.0
        ),
        "codex_reduction_target": (
            metrics["minimum_codex_usage_reduction_fraction"] >= 0.7
        ),
        "zero_duplicates": (
        ...
        "full_local_gpu_release": (
            metrics["minimum_local_gpu_release_fraction"] == 1.0
        ),
    ...
    result = {
        ...
        "campaign_telemetry_sha256": [
            canonical_sha256(record["telemetry"]) for record in records
        ],
        "campaign_slo_sha256": [
            record["slo"]["result_sha256"] for record in records
        ],
        "campaign_handoff_audit_sha256": [
...
def validate_sustained_campaign_slo_replay(
    ...
    *,
    ...
    if not isinstance(result.get("metrics"), Mapping) or set(
        result["metrics"]
    ) != _METRIC_FIELDS:
        raise SustainedCampaignSloError("sustained SLO metric field mismatch")
    if not isinstance(result.get("gates"), Mapping) or set(
        result["gates"]
    ) != _GATE_FIELDS:
    ...
    if dict(result) != expected:
        raise SustainedCampaignSloError(
            "sustained SLO deterministic replay mismatch"
        )

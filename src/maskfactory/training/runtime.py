...
def load_training_stack_lock(path: Path = DEFAULT_LOCK_PATH) -> dict[str, Any]:
    ...
    accepted = runtime.get("accepted_compute_capabilities", [[12, 0]])
    if (
        not isinstance(accepted, list)
        or not accepted
        or any(
            (not isinstance(item, list) or len(item) != 2 or item != [int(item[0]), int(item[1])])
            for item in accepted
        )
        or [12, 0] not in accepted
    ):
        raise TrainingRuntimeError(
            "OpenMMLab lock accepted_compute_capabilities must include [12, 0]"
        )
...
def evaluate_openmmlab_runtime(
    *,
    ...
    accepted_capabilities = {
        ...
        for item in lock["runtime"].get(
            "accepted_compute_capabilities",
            [lock["runtime"]["compute_capability"]],
        )
    }
    if cuda_capability not in accepted_capabilities:
        issues.append(
            "CUDA capability mismatch: "
            f"expected one of {sorted(accepted_capabilities)}, observed {cuda_capability}"
        )
...
def probe_openmmlab_runtime(path: Path = DEFAULT_LOCK_PATH) -> TrainingRuntimeReport:
    ...
    if all(versions[key] == lock["packages"][key]["version"] for key in REQUIRED_PACKAGES):
        ...
        try:
            ...
            importlib.import_module("maskfactory.training.mmseg_transforms")
            registry_module = importlib.import_module("mmseg.registry")
            transforms_registered = all(
                name in registry_module.TRANSFORMS.module_dict
                for name in (

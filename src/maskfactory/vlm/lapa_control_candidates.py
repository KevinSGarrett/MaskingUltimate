...
def _document(path: Path, *, yaml_input: bool = False) -> Mapping[str, Any]:
    try:
        value = yaml.safe_load(path.read_text(encoding="utf-8")) if yaml_input else json.loads(path.read_text(encoding="utf-8"))
...
def _dedup_index(document: Mapping[str, Any]) -> dict[str, dict[str, str]]:
    ...
    if document.get("seal_sha256") is not None and document["seal_sha256"] != seal_payload(document):
...
def _assert_policy(provenance: Mapping[str, Any], remap: Mapping[str, Any]) -> list[str]:
    ...
    for value in sorted(HEAD_FACE_VALUES):
        ...
        if not isinstance(entry, Mapping) or entry.get("action") != "merge" or entry.get("part") != ["head_face"] or not isinstance(entry.get("source_label"), str):
            raise LaPaControlCandidateError(f"LaPa head-face remap drift:{value}")
        names.append(entry["source_label"])
    return names


def _candidate(root: Path, image: Path, manifest: Mapping[str, str], dedup: Mapping[str, Mapping[str, str]], names: list[str]) -> dict[str, Any] | None:
    ...
    return {
        "sample_id": f"{source_id}_head_face", "source_image_id": source_id,
        "canonical_label": "head_face", "raw_label_values": sorted(HEAD_FACE_VALUES), "raw_label_names": names,
        "assigned_partition": _partition(split), "upstream_split": split, "split_group_id": lineage["split_group_id"],
        "source_relative_path": image_relative, "source_sha256": image_sha, "source_dimensions": image_size,
        "indexed_label_relative_path": label_relative, "indexed_label_sha256": label_sha, "indexed_label_dimensions": label_size,
        "indexed_label_values": values, "mask_pixel_count": int(np.count_nonzero(mask)),
        "alignment_policy": "source_and_indexed_label_geometry_must_match",
        "external_reference_qualification_complete": True, "visual_alignment_reviewed": False,
        "critic_control_eligible": False, "gold_or_production_authority": False,
    }


def build_lapa_control_candidates(*, source_root: Path, project_root: Path, provenance_path: Path, inventory_path: Path, remap_path: Path, qualification_evidence_path: Path, source_hash_manifest_path: Path, split_dedup_path: Path, per_partition: int = 8) -> dict[str, Any]:
    ...
    provenance, inventory, remap = _document(Path(provenance_path), yaml_input=True), _document(Path(inventory_path)), _document(Path(remap_path), yaml_input=True)
    bundle, manifest_document, dedup_document = _document(Path(qualification_evidence_path)), _document(Path(source_hash_manifest_path)), _document(Path(split_dedup_path))
    names = _assert_policy(provenance, remap)
    qualification = verify_external_qualification_evidence(provenance, inventory, source="lapa", evidence_bundle=bundle, project_root=project)
    if not qualification.admitted:
        raise LaPaControlCandidateError("LaPa external qualification is not admitted:" + ",".join(qualification.unmet_gates or qualification.evidence_tokens))
    ...
    for partition in PARTITIONS:
        ranked = sorted(pools[partition], key=lambda row: canonical_sha256({"source_relative_path": row["source_relative_path"], "source_sha256": row["source_sha256"], "indexed_label_sha256": row["indexed_label_sha256"]}))
    ...
    result: dict[str, Any] = {
        "schema_version": SCHEMA_VERSION, "artifact_type": "lapa_exact_head_face_control_candidates",
        "input_bindings": {"provenance_sha256": sha256_file(Path(provenance_path)), "inventory_sha256": sha256_file(Path(inventory_path)), "remap_sha256": sha256_file(Path(remap_path)), "qualification_evidence_sha256": sha256_file(Path(qualification_evidence_path)), "qualification_evidence_bundle_sha256": qualification.evidence_bundle_sha256, "source_hash_manifest_sha256": sha256_file(Path(source_hash_manifest_path)), "source_hash_manifest_seal_sha256": manifest_document["seal_sha256"], "split_dedup_sha256": sha256_file(Path(split_dedup_path))},
        "selection_policy": {"canonical_label": "head_face", "exact_source_values": sorted(HEAD_FACE_VALUES), "upstream_partition_mapping": {"train": "qualification_train", "val": "qualification_test", "test": "qualification_test"}, "partitions": list(PARTITIONS), "per_partition": per_partition, "selection_order": "sha256(source_relative_path,source_sha256,indexed_label_sha256)", "split_group_disjoint": True},
        "selected_count": len(selected), "selected_by_label": {"head_face": len(selected)}, "selected_by_partition": dict(sorted(by_partition.items())), "selected": selected,
        "authority_claimed": False, "critic_control_authority_granted": False, "gold_or_production_authority_granted": False,
        "claim_limits": ["LaPa remains qualified external weighted pseudo-label supervision, never gold or holdout truth.", "Candidates are calibration-only and require per-record panel screening under Amendment 2.", "Exact head-face semantics are limited to the governed LaPa 1..9 merge.", "No critic role, certificate, production-mask, package, or training-truth authority is granted."],
...
def verify_lapa_control_candidates(document: Mapping[str, Any]) -> None:
    ...
    if document.get("self_sha256") != canonical_sha256({key: value for key, value in document.items() if key != "self_sha256"}):
        raise LaPaControlCandidateError("candidate self-hash drift")
    if document.get("schema_version") != SCHEMA_VERSION or document.get("artifact_type") != "lapa_exact_head_face_control_candidates" or document.get("authority_claimed") is not False or document.get("critic_control_authority_granted") is not False or document.get("gold_or_production_authority_granted") is not False:
        raise LaPaControlCandidateError("candidate authority or schema drift")
    bindings = document.get("input_bindings")
    expected = {"provenance_sha256", "inventory_sha256", "remap_sha256", "qualification_evidence_sha256", "qualification_evidence_bundle_sha256", "source_hash_manifest_sha256", "source_hash_manifest_seal_sha256", "split_dedup_sha256"}
    ...
    for row in rows:
        ...
        sample_id, group, partition = row.get("sample_id"), row.get("split_group_id"), row.get("assigned_partition")
        if not isinstance(sample_id, str) or not sample_id or sample_id in ids or not isinstance(group, str) or not group or group in groups or partition not in PARTITIONS or row.get("canonical_label") != "head_face" or row.get("raw_label_values") != sorted(HEAD_FACE_VALUES) or row.get("external_reference_qualification_complete") is not True or row.get("visual_alignment_reviewed") is not False or row.get("critic_control_eligible") is not False or row.get("gold_or_production_authority") is not False or partition != _partition(str(row.get("upstream_split"))):
        ...
        if not isinstance(dimensions, list) or dimensions != row.get("indexed_label_dimensions") or len(dimensions) != 2 or any(not isinstance(value, int) or value <= 0 for value in dimensions) or not isinstance(row.get("mask_pixel_count"), int) or row["mask_pixel_count"] <= 0:
            raise LaPaControlCandidateError("candidate geometry drift")
        groups.add(group)
        ids.add(sample_id)
        counts[partition] += 1
    if document.get("selected_by_label") != {"head_face": len(rows)} or document.get("selected_by_partition") != dict(sorted(counts.items())):
        raise LaPaControlCandidateError("candidate summary count drift")


def materialize_lapa_control_panels(*, source_root: Path, candidate_document: Mapping[str, Any], output_root: Path) -> dict[str, Any]:
    ...
    try:
        ...
        for candidate in candidate_document["selected"]:
            ...
            if sha256_file(image) != candidate["source_sha256"] or sha256_file(label) != candidate["indexed_label_sha256"]:
            ...
            with Image.open(label) as opened:
                indexed = np.asarray(opened.convert("L"), dtype=np.uint8)
            if list(source.shape[1::-1]) != candidate["source_dimensions"] or list(indexed.shape[1::-1]) != candidate["indexed_label_dimensions"] or source.shape[:2] != indexed.shape or sorted(int(value) for value in np.unique(indexed)) != candidate["indexed_label_values"]:
            ...
            rows.append({**candidate, "source_path_runpod": image.as_posix(), "indexed_label_path_runpod": label.as_posix(), "source_encoded_sha256_verified": True, "indexed_label_sha256_verified": True, "head_face_remap_pixels_verified": True, **panels, "visual_alignment_reviewed": False, "critic_control_eligible": False, "gold_or_production_authority": False})
        width, height, columns = 720, 260, 4
        contact = Image.new("RGB", (width * columns, height * ((len(rows) + columns - 1) // columns)), color=(18, 18, 18))
        ...
        report: dict[str, Any] = {"schema_version": PANEL_SCHEMA_VERSION, "artifact_type": "lapa_exact_head_face_visual_evidence", "authority_claimed": False, "visual_alignment_qualification_complete": False, "critic_control_authority_granted": False, "candidate_set_sha256": candidate_document["self_sha256"], "record_count": len(rows), "panel_count": len(rows) * len(PANEL_NAMES), "panels_per_record": list(PANEL_NAMES), "contact_sheet": {"path": "contact_sheet.png", "sha256": sha256_file(contact_path), "scheduling_and_navigation_aid_only": True, "per_record_evidence_required": True}, "records": rows, "next_required_stage": "per_record_visual_alignment_and_control_admission_screening", "claim_limits": ["Exact indexed-map rendering does not complete per-record control admission.", "Contact sheets are scheduling and navigation aids only.", "No critic-control, gold, certificate, package, or production authority."]}
...
def verify_lapa_control_panel_report(document: Mapping[str, Any], root: Path) -> None:
    ...
    if document.get("schema_version") != PANEL_SCHEMA_VERSION or document.get("artifact_type") != "lapa_exact_head_face_visual_evidence" or document.get("authority_claimed") is not False or document.get("visual_alignment_qualification_complete") is not False or document.get("critic_control_authority_granted") is not False:
    ...
    for row in rows:
        if row.get("visual_alignment_reviewed") is not False or row.get("critic_control_eligible") is not False or row.get("gold_or_production_authority") is not False:

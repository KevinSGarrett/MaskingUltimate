...
def build_lv_mhp_direct_control_candidates(
    *,
    ...
    source_value, source_label, canonical_label = (
        _value(source_value),
        _label(source_label, "source"),
        _label(canonical_label, "canonical"),
    )
    if per_partition < 1:
        raise LvMhpDirectControlCandidateError("per-partition must be positive")
    content, provenance, inventory, remap = (
        _content(source_root),
    ...
    entry = (
        mappings.get(source_value, mappings.get(str(source_value)))
    ...
    if not qualified.admitted:
        raise LvMhpDirectControlCandidateError(
    ...
    for image_id, identity in identities.items():
        ...
        for instance in identity["instance_ids"]:
            ...
            if dimensions != [int(indexed.shape[1]), int(indexed.shape[0])] or not set(
                values
            ).issubset(set(range(19))):
            ...
            pools[PARTITIONS[lineage["split"]]].append(
                {
                    "sample_id": f"lv_mhp_v1_{image_id}_p{instance:02d}_{canonical_label}",
                    "source_image_id": f"lv_mhp_v1_{image_id}",
                    "source_instance_id": instance,
                    "declared_person_count": identity["person_count"],
                    "canonical_label": canonical_label,
                    "raw_label_values": [source_value],
                    "raw_label_names": [source_label],
                    ...
                    "instance_annotation_dimensions": dimensions,
    ...
    for partition in PARTITIONS.values():
        ...
        if len(chosen) != per_partition:
            raise LvMhpDirectControlCandidateError(
                f"insufficient split-disjoint direct candidates:{canonical_label}:{partition}"
            )
    ...
    result: dict[str, Any] = {
        ...
        "artifact_type": "lv_mhp_v1_exact_direct_control_candidates",
        "input_bindings": {
        ...
        "selection_policy": {
            ...
            "exact_source_values": [source_value],
        ...
        "selected_by_label": {canonical_label: len(selected)},
        ...
        "claim_limits": [
            "LV-MHP is external weighted pseudo-label supervision, never gold or holdout truth.",
            "Candidates are calibration-only and require individual Amendment-2 panel screening.",
            "Only this exact direct remap is eligible; split-required labels are rejected.",
            "No critic role, certificate, package, production, or training-truth authority is granted.",
        ],
        "next_required_stage": "render_exact_panels_then_per_record_control_admission",
    }
...
def verify_lv_mhp_direct_control_candidates(document: Mapping[str, Any]) -> None:
    ...
    if (
        ...
        or document.get("artifact_type") != "lv_mhp_v1_exact_direct_control_candidates"
    ...
    source_value, source_label, canonical_label = (
        _value(values[0]),
        _label(policy.get("source_label"), "policy source"),
        _label(policy.get("canonical_label"), "policy canonical"),
    )
    ...
    for row in rows:
        if (
            ...
            or row.get("canonical_label") != canonical_label
            or row.get("raw_label_values") != [source_value]
            or row.get("raw_label_names") != [source_label]
    ...
    if document.get("selected_by_label") != {canonical_label: len(rows)} or document.get(

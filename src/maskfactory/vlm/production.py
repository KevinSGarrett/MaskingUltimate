        ...
        for label, candidate_inputs in sorted(autonomy_inputs.items()):
            ...
            try:
                emit = emit_lifecycle_and_corpus_record(
                    ...
                    machine_root=os.environ.get("MASKFACTORY_MACHINE_ROOT"),
                    risk_bucket=autonomy_context,
                )
                autonomy_lifecycle.append(
                    {
                        "status": emit["lifecycle_status"],
                        "image_id": str(report["image_id"]),
                        "label": label,
                        "corpus_envelope_written": emit["corpus_envelope_written"],
                        "lifecycle_path": emit.get("lifecycle_relpath"),
                    }
                )
            except (AutonomyEmitError, OSError, ValueError):
                # Drafts outside production runs/ still get a review lifecycle sidecar.

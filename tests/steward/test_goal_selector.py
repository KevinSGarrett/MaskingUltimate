...
def test_repeated_no_progress_switches_to_next_unblocked_lane():
    data = _tracker()
    _set_plan27_statuses(data)
    data["items"]["MF-P6-14.01"]["status"] = "open"
    data["items"]["MF-P6-14.02"]["status"] = "open"
...
def test_superseded_item_is_not_reissued():
    data = _tracker()
    _set_plan27_statuses(data)
    data["items"]["MF-P6-14.01"]["status"] = "open"
    data["items"]["MF-P6-14.02"]["status"] = "open"
    ...
    decision = select_next_plan27_work(
        data,
        inference_available=True,
        superseded_item_ids={"MF-P6-14.01"},
    )
...
def test_returns_none_when_no_eligible_work_exists():
    data = _tracker()
    _set_plan27_statuses(data)

    assert select_next_plan27_work(data, inference_available=False) is None

...
def _candidate(
    ...
    return AnatomyDraftCandidate(
        ...
        mask,
        confidence,
        "sam2_refined_non_gold_review_draft" if mask is not None else "suppressed_for_human_review",
        f"review {label}",
        {
            "adult_intake_decision": "clear_adult",
            "sam2_required": True,
...
def test_inactive_config_is_canonical_visible_only_and_clear_adult_gated() -> None:
    ...
    assert config["governance"]["allowed_intake_decisions"] == ["clear_adult"]
...
def test_review_crops_and_prompts_assert_no_positive_or_hidden_anatomy() -> None:
    ...
    requests = canonical_open_vocab_requests(crops, intake_decision="clear_adult")
    ...
    with pytest.raises(AnatomyV2DraftError, match="requires clear_adult"):
        canonical_open_vocab_requests(crops, intake_decision="uncertain_age")
...
def test_detector_box_must_route_through_sam2_and_low_confidence_never_uses_prior() -> None:
    ...
    assert candidate.authority == "sam2_refined_non_gold_review_draft"

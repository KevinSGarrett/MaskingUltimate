...
def _clone(inputs: AnatomyV2QaInputs, **overrides) -> AnatomyV2QaInputs:
    values = {
        ...
        "adult_intake_decision": inputs.adult_intake_decision,
...
def _seed_defect(inputs: AnatomyV2QaInputs, qc_id: str) -> AnatomyV2QaInputs:
    ...
    if qc_id == "QC-V2-001":
        seeded.manifest["parts"].pop("head_face")
        ...
        seeded = _clone(seeded, adult_intake_decision="uncertain")
...
def test_good_seed_passes_all_twelve_and_report_remains_non_authoritative(tmp_path: Path) -> None:
...
def test_each_seeded_defect_trips_its_named_v2_check(qc_id: str) -> None:
    results = {
        result.qc_id: result for result in run_anatomy_v2_qc(_seed_defect(_good_inputs(), qc_id))
    }
    assert results[qc_id].passed is False, results[qc_id]

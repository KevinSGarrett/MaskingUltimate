...
def test_cloud_prompt_spells_out_atomic_mtp_contract(tmp_path: Path):
    request, _mask, _protected = _fixture(tmp_path)
    foot_request = TeacherRequest(
        request.image_id,
        request.instance_id,
        "right_foot_base",
        request.source_path,
    ...
    prompt = _teacher_prompt(foot_request, "Audit <label>")

    assert "foot base excludes toes beginning at metatarsophalangeal line" in prompt
    assert "Never pass a whole hand as hand_base or a whole bare foot as foot_base" in prompt
...
def test_cloud_teacher_config_is_authorized_shadow_only_and_capped_at_fifteen_dollars():
    ...
    assert config["budget"]["maximum_calls_per_job"] == 9
    assert config["budget"]["maximum_calls_per_label"] == 6
...
def test_teacher_committee_calls_every_enabled_provider_for_the_exact_candidate(
    tmp_path: Path,
):
    request, _mask, _protected = _fixture(tmp_path)
    config = load_cloud_teacher_config()
    ...
    config["governance"]["eligibility_registry"] = str(registry)
    calls = []
    ...
    class Provider:
        ...
        def review(self, reviewed_request, prompt):
            calls.append(
                (
                    self.name,
                    tuple(str(path) for path in reviewed_request.evidence.images),
                    prompt,
                )
            )
            return parse_teacher_judgment(
                json.dumps(_teacher_response(verdict="pass", tool="none")),
                provider=self.name,
                model=self.model,
                usage=TeacherUsage(100, 20, 0.01),
                latency_ms=1,
            )

    budget = DailyBudgetLedger(
        tmp_path / "committee_costs.jsonl", timezone_name="UTC", hard_limit_usd="2"
    )
    judgments = run_teacher_committee(
        request,
        providers={name: Provider(name) for name in ("gemini", "openai", "anthropic")},
        config=config,
        budget=budget,
        prompt_template=Path("src/maskfactory/vlm/prompts/p_cloud_teacher.txt").read_text(),
        report_path=tmp_path / "committee.json",
    )

    assert [item.provider for item in judgments] == ["gemini", "openai", "anthropic"]
    assert [item[0] for item in calls] == ["gemini", "openai", "anthropic"]
    assert len({item[1] for item in calls}) == 1
    assert float(budget.snapshot().committed_usd) == pytest.approx(0.03)
...
def test_definitely_unbilled_provider_rejection_releases_reservation(tmp_path: Path):
    request, _mask, _protected = _fixture(tmp_path)
    config = load_cloud_teacher_config()
    config["enabled"] = True
    config["providers"]["gemini"]["enabled"] = True
    ...
    class QuotaRejectedProvider:
        name, model, maximum_reserved_cost_usd = "gemini", "quota-rejected", 0.2

        def review(self, request, prompt):
            raise CloudProviderRequestError("HTTP 429", definitely_unbilled=True)

    budget = DailyBudgetLedger(tmp_path / "costs.jsonl", timezone_name="UTC", hard_limit_usd="1")
    assert (
        run_teacher_cascade(
            request,
            providers={"gemini": QuotaRejectedProvider()},
            config=config,
            budget=budget,
            prompt_template="Review <label>",
            report_path=tmp_path / "report.json",
    ...
    assert budget.snapshot().committed_usd == 0 and budget.snapshot().reserved_usd == 0
    assert "definitely_unbilled_transport_failure" in (tmp_path / "costs.jsonl").read_text()


def test_cloud_job_quota_caps_total_and_per_label_calls() -> None:
    quota = CloudJobQuota(maximum_calls_per_job=3, maximum_calls_per_label=2)
    quota.claim("right_foot_base")
    quota.claim("right_foot_base")
    with pytest.raises(CloudTeacherError, match="per label"):
        quota.claim("right_foot_base")
    quota.claim("right_toes")
    with pytest.raises(CloudTeacherError, match="per S11 job"):
        quota.claim("left_foot_base")
...
def test_tight_boundary_points_are_additive_and_retry_missed_positive_points(tmp_path: Path):
    request, mask, protected = _fixture(tmp_path)
    response = _teacher_response(tool="points")
    response["defects"] = ["boundary_too_tight"]
    response["correction"]["positive_points"] = [[700, 500], [850, 500]]
    response["correction"]["negative_points"] = []
    judgment = parse_teacher_judgment(
        json.dumps(response),
        provider="gemini",
        model="g",
    ...
    class Refiner:
        ...
        def refine_roi(self, image, label, clicks, *, roi_xyxy):
            self.calls += 1
            output = np.zeros(mask.shape, dtype=bool)
            selected = clicks[:1] if len(clicks) > 1 else clicks
            for point in selected:
                output[20:40, 50 : int(point["x"]) + 2] = True
            return output
    ...
    candidate = materialize_teacher_candidate(
        ...
        protected_neighbor=protected,
        refiner=refiner,
        output_path=tmp_path / "additive.png",
    )
    assert candidate.status == "candidate_created" and refiner.calls == 2
    repaired = np.asarray(Image.open(tmp_path / "additive.png")) > 0
    assert np.all(repaired[mask]) and repaired.sum() > mask.sum()
...
@pytest.mark.parametrize(
    ("provider_class", "name", "env_name", "wrap"),
    [
        (GeminiTeacherProvider, "gemini", "GEMINI_API_KEY", "gemini"),
        (AnthropicTeacherProvider, "anthropic", "ANTHROPIC_API_KEY", "anthropic"),
    ],
)
def test_provider_recovers_wrapped_json_without_a_second_billable_call(
    tmp_path: Path, monkeypatch, provider_class, name, env_name, wrap
):
    request, _mask, _protected = _fixture(tmp_path)
    raw = "```json\n" + json.dumps(_teacher_response(verdict="pass", tool="none")) + "\n```"
    ...
    judgment = provider.review(request, "prompt")
    assert judgment.verdict == "pass" and transport.calls == 1


def test_provider_makes_one_schema_repair_call_and_aggregates_usage(tmp_path: Path, monkeypatch):
    request, _mask, _protected = _fixture(tmp_path)
    ...
    class Transport:
        ...
        def post(self, url, *, headers, payload, timeout):
            ...
            raw = (
                "not-json"
                if self.calls == 1
                else json.dumps(_teacher_response(verdict="pass", tool="none"))
            )
            return {
                "candidates": [{"content": {"parts": [{"text": raw}]}}],
                "usageMetadata": {"promptTokenCount": 100, "candidatesTokenCount": 50},
    ...
    assert judgment.usage.input_tokens == 200 and judgment.usage.output_tokens == 100
...
def test_learning_records_refuse_drafts_and_accept_only_frozen_human_gold(tmp_path: Path):
    ...
    baseline = mask.copy()
    baseline[4, 5] = True
    write_binary_mask(package / "annotations/draft_baseline/masks/left_forearm.png", baseline)
    review_draft_dir = package / "annotations/autonomy/autonomy_review_draft"
    review_draft_dir.mkdir(parents=True)
    review_map = np.zeros((20, 20), dtype=np.uint16)
    review_map[mask] = 18
    write_label_map(review_draft_dir / "label_map_part.png", review_map, bits=16)
    manifest = {
        "image_id": "img_0123456789ab",
        "review": {
            "reviewer": "kevin",
    ...
    assert record["baseline_changed_pixels_to_gold"] == 1
    assert record["review_draft_changed_pixels_to_gold"] == 0
    assert record["machine_changed_pixels_from_baseline"] == 1
    assert record["human_correction_pixels_avoided"] == 1
    assert record["review_time_sec"] == 90
    harvest_human_teacher_resolution(
        package_root=package,
        teacher_report_path=teacher_report,
        output_path=tmp_path / "learning.jsonl",
...
def test_s11_cloud_teacher_is_shadow_only_and_writes_isolated_candidate(tmp_path: Path):
    ...
    config = load_cloud_teacher_config()
    config["enabled"] = True
    config["selection"]["diagnosis_cascade_before_autonomous_convergence"] = True
    config["providers"]["gemini"]["enabled"] = True
    config["selection"]["always_escalate_labels"].append("left_forearm")
    config["budget"]["ledger_path"] = str(tmp_path / "costs.jsonl")
    config["governance"]["eligibility_registry"] = str(tmp_path / "eligibility.yaml")
...
def test_s11_promotes_qa_clean_local_correction_into_non_gold_review_draft(
    ...
    repair_hints = tmp_path / "prompts.json"
    repair_hints.write_text(
        json.dumps(
            {
                "schema_version": "1.0.0",
                "plans": [
                    {
                        "label": "left_forearm",
                        "box_xyxy": [25, 15, 47, 35],
                        "prior_quality": "pose_chain",
                    }
                ],
            }
        )
            ...
            return json.dumps(
                {
                    ...
                    "correction_plan": {
                        ...
                        "positive_points": [[35, 25]] if verdict == "fail" else [],
                        "negative_points": [[45, 25]] if verdict == "fail" else [],
    ...
    status = run_s11_production(
        ...
        repair_hints_path=repair_hints,
        person_bbox_xyxy=(10, 5, 70, 55),
    )
    ...
    assert status["autonomy"]["convergence"]["left_forearm"]["converged"] is True
    assert (
        status["autonomy"]["convergence"]["left_forearm"]["stop_reason"]
        == "all_required_reviewers_passed_exact_candidate"
    )
    assert validation_tags == [
        "left_forearm_local_r1",
        "left_forearm_committee_r1",
        "autonomy_review_draft_left_forearm_local_correction_r1",
        "autonomy_review_draft_final",
    ]
    assert np.count_nonzero(draft == 18) == 11 * 10
    assert np.array_equal(np.asarray(Image.open(part_path)), part)

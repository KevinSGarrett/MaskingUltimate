...
def test_default_style_doctor_short_circuits_repeated_wsl_failures() -> None:
    ...
    with (
        patch("maskfactory.doctor._windows_host", return_value=True),
...
def test_data_junction_fails_when_resolved_onto_policy_usb_f(tmp_path: Path) -> None:
    data = tmp_path / "data"
    data.mkdir()
    # PureWindowsPath keeps .drive parseable when these hermetic tests run on Linux.
    usb_target = PureWindowsPath("F:/MaskFactory_DataRelocated")
    with (
...
def test_data_junction_fails_when_getdrivetype_removable(tmp_path: Path) -> None:
    data = tmp_path / "data"
    data.mkdir()
    target = PureWindowsPath("E:/external_data")
    with (
        patch("maskfactory.doctor.os.name", "nt"),
...
def test_data_junction_passes_on_fixed_local_target(tmp_path: Path) -> None:
    data = tmp_path / "data"
    data.mkdir()
    fixed = PureWindowsPath("C:/Comfy_UI_Main_Masking/data_c_backup_relocated")
    with (
        patch("maskfactory.doctor.os.name", "nt"),
...
def test_cvat_project_skip_is_only_allowed_before_p1() -> None:
    with (
        ...
        patch("maskfactory.doctor._cvat_nuclio_runtime_blocked", return_value=None),
    ):
...
def test_native_linux_skips_cvat_nuclio_when_docker_cannot_create_containers() -> None:
    blocked = (
        "RUNTIME_BLOCKED_POD_CLASS: no docker.sock; nested RunPod pod cannot bootstrap CVAT compose"
    )
    with (
        patch("maskfactory.doctor._windows_host", return_value=False),
        patch("maskfactory.doctor._cvat_nuclio_runtime_blocked", return_value=blocked),
        patch("maskfactory.doctor._env_values", return_value={}),
        patch("maskfactory.doctor._p1_started", return_value=True),
    ):
        for check in (check_cvat_api, check_cvat_project, check_nuclio_interactor):
            result = check()
            assert result.status == "SKIP"
            assert "RUNTIME_BLOCKED_POD_CLASS" in result.detail
            assert "bare-metal/privileged" in result.hint
...
def test_wsl_identity_failure_is_readable_and_actionable(tmp_path: Path) -> None:
    ...
    with (
        # Patch the host helper — do not flip os.name (breaks pathlib.Path on Linux).
        patch("maskfactory.doctor._windows_host", return_value=True),
...
def test_native_linux_skips_wsl_preflight_and_roundtrip() -> None:
    calls = []

    def torch_check() -> CheckResult:
        calls.append("torch")
        return CheckResult("torch_cuda", "PASS", "native")

    def models_check() -> CheckResult:
        calls.append("models")
        return CheckResult("registered_models", "PASS", "native")

    torch_check.__name__ = "check_torch_cuda"
    models_check.__name__ = "check_registered_models"
    with (
        patch("maskfactory.doctor.os.name", "posix"),
        patch("maskfactory.doctor._wsl_preflight_failure") as preflight,
    ):
        results = run_doctor(
            [torch_check, models_check, check_wsl_roundtrip],
            preflight_wsl=None,
        )
    preflight.assert_not_called()
    assert calls == ["torch", "models"]
    assert [result.status for result in results] == ["PASS", "PASS", "SKIP"]
    assert results[2].name == "wsl_roundtrip"

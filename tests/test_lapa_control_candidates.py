...
def _inputs(tmp_path: Path) -> tuple[Path, Path, Path, Path, Path, Path, Path]:
            ...
            records.append(
                {
                    ...
                    "split_group_id": f"group-{split}-{number}",
    ...
    provenance = {
        ...
        "sources": {"lapa": {"training_admission": {"allowed_label_scope": ["head_face", "hair"]}}},
    }
    mappings = {
        0: {"source_label": "background", "action": "direct", "part": ["background"]},
        10: {"source_label": "hair", "action": "direct", "part": ["hair"]},
    }
    for value in range(1, 10):
        mappings[value] = {
            "source_label": f"face_{value}",
            "action": "merge",
            "part": ["head_face"],
        }
    ...
    remap_path.write_text(
        yaml.safe_dump({"source": "lapa", "mappings": mappings}), encoding="utf-8"
    )
...
def _qualified(admitted: bool = True) -> QualificationEvidence:
    return QualificationEvidence(
        ...
        technically_qualified=admitted,
        admitted=admitted,
        unmet_gates=() if admitted else ("source_hash_manifested",),
        evidence_tokens=(),
        evidence_bundle_sha256="a" * 64 if admitted else None,
        reason="fixture",
    )
...
def _build(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> tuple[dict, Path]:
    ...
    monkeypatch.setattr(
        lapa, "verify_external_qualification_evidence", lambda *args, **kwargs: _qualified()
    )
    return (
        lapa.build_lapa_control_candidates(
            source_root=root,
            project_root=tmp_path,
            provenance_path=provenance,
            inventory_path=inventory,
            remap_path=remap,
            qualification_evidence_path=bundle,
            source_hash_manifest_path=manifest,
            split_dedup_path=dedup,
            per_partition=4,
        ),
        root,
    )
...
def test_selects_split_disjoint_exact_head_face_candidates(
...
def test_cross_split_identity_group_is_excluded(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    root, provenance, inventory, remap, bundle, manifest, dedup = _inputs(tmp_path)
    document = json.loads(dedup.read_text(encoding="utf-8"))
    train = next(
        row for row in document["records"] if row["relative_path"] == "train/images/train_00.jpg"
    )
    test = next(
        row for row in document["records"] if row["relative_path"] == "test/images/test_00.jpg"
    )
    ...
    result = lapa.build_lapa_control_candidates(
        source_root=root,
        project_root=tmp_path,
        provenance_path=provenance,
        inventory_path=inventory,
...
def test_external_qualification_failure_fails_closed(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    root, provenance, inventory, remap, bundle, manifest, dedup = _inputs(tmp_path)
    monkeypatch.setattr(
        lapa, "verify_external_qualification_evidence", lambda *args, **kwargs: _qualified(False)
    )
...
def test_materializes_and_verifies_exact_panels(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    candidates, root = _build(monkeypatch, tmp_path)
    output = tmp_path / "panels"
    report = lapa.materialize_lapa_control_panels(
        source_root=root, candidate_document=candidates, output_root=output
    )
...
def test_authority_upgrade_fails_panel_verification(
    monkeypatch: pytest.MonkeyPatch, tmp_path: Path
) -> None:
    candidates, root = _build(monkeypatch, tmp_path)
    output = tmp_path / "panels"

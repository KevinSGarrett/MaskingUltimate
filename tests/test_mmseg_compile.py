...
def test_compile_segformer_is_self_contained_and_exact(tmp_path: Path) -> None:
    ...
    assert compiled["model"]["type"] == "MaskFactoryWeightedEncoderDecoder"
    assert "maskfactory.training.weighted_segmentor" in compiled["custom_imports"]["imports"]
...
def test_compile_mask2former_uses_native_matcher_and_checkpointed_swin_b(
    ...
    assert model["type"] == "MaskFactoryWeightedEncoderDecoder"

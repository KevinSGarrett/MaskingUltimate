...
def test_v2_failure_reasons_return_canonical_fail_closed_acquisition_actions(
    ...
    for reason, category in FAILURE_ACQUISITION_CATEGORIES.items():
        first = acquisition_action_for_v2_failure(reason, label="left_areola")
        second = acquisition_action_for_v2_failure(reason, label="left_areola")
        assert first == second
        assert first["failure_reason"] == reason
        assert first["category"] == category
        assert first["action"]
        assert first["required_review_states"]

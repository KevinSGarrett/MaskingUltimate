...
def _promoted_critic_catalog() -> dict:
...
def _critic_role_certificate(catalog: dict, model_index: int, role: str) -> dict:
...
def _prepare(tmp_path: Path, *, instance_context: str = "solo") -> dict:
    ...
    critic_catalog = _promoted_critic_catalog()
    critic_role_certificates = (
        _critic_role_certificate(critic_catalog, 5, "primary_visual_critic"),
        _critic_role_certificate(critic_catalog, 3, "independent_juror"),
    )
    critic_decision = CriticQuorumDecision(
        status="pass",
        independent_families=tuple(
            sorted(certificate["family_id"] for certificate in critic_role_certificates)
        ),
        ...
        family_verdicts={
            ...
            for family_id in sorted(
                certificate["family_id"] for certificate in critic_role_certificates
            )
        },
        blockers=(),
    ...
    bound_critic_decision = bind_catalog_critic_quorum(
        critic_decision,
        critic_catalog=critic_catalog,
        critic_role_certificates=critic_role_certificates,
        decision_time="2026-07-17T00:00:05Z",
    )
    certificate["qa_evidence"]["critic_report_sha256"] = critic_quorum_sha256(bound_critic_decision)
...
def test_issuer_requires_exact_independent_critic_quorum_without_critic_authority(
    ...
    assert "catalog_critic_runtime_quorum_mismatch" in caught.value.codes
...
def test_operational_policy_failures_precede_critic_failures(tmp_path: Path) -> None:
    prepared = _prepare(tmp_path)
    failed = replace(
        prepared["critic_quorum_decision"],
        status="autonomous_abstention",
    ...
    with pytest.raises(OperationalCertificateIssuanceError) as caught:
        _issue(
            prepared,
            critic_quorum_decision=failed,
            trusted_operational_policy_evaluators={},
        )
    assert caught.value.codes == ("operational_policy_evaluator_untrusted",)

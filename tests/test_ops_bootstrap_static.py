...
def test_dvc_config_binds_expected_remote() -> None:
    ...
    assert checks["remote_name"] == "maskfactory-dvc-local"
    assert checks["remote_url"] == "F:/MaskFactory_DataRelocated/dvc_local_remote"
...
def test_bootstrap_overclaim_fail_closed() -> None:
    ...
    with pytest.raises(OpsBootstrapStaticError, match="dvc_persistent_push_succeeded"):
        refuse_bootstrap_overclaim({"dvc_persistent_push_succeeded": True})
...
def test_suite_seals_schema_valid_static_report() -> None:
    ...
    assert report["dvc_persistent_push_succeeded"] is False
    assert report["dvc_push_attempted"] is False
    assert report["literal_data_packages_push_required"] is True
    ...
    assert report["dvc_runtime"]["remote_url"] == "F:/MaskFactory_DataRelocated/dvc_local_remote"

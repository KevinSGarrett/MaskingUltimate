...
def test_matrix_rejects_gated_checkpoint_claim_as_qualified(tmp_path: Path):
    document = json.loads(MATRIX.read_text(encoding="utf-8"))
    ...
    row.pop("needs_kevin")
    _rehash(document)
    with pytest.raises(RuntimeMatrixError, match="runtime_expected_human_gate:sam3_1"):

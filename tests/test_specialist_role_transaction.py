...
from maskfactory.models.registry import (
    ...
    GENESIS_PREVIOUS_LIFECYCLE,
    GENESIS_PREVIOUS_ROLE,
    ModelRegistryError,
...
def test_specialist_promotion_and_one_command_rollback_are_hash_sealed(
    ...
    assert record["install_kind"] == "incumbent_swap"
...
def test_first_role_install_uses_genesis_sentinel_without_force_promote(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Governed bootstrap: empty champion slot + genesis-bound packet installs first champion."""
    fixture = _fixture(tmp_path, monkeypatch)
    document = json.loads(fixture["registry"].read_text(encoding="utf-8"))
    document["models"] = [row for row in document["models"] if row["key"] != "incumbent_hand"]
    fixture["registry"].write_text(json.dumps(document, indent=2), encoding="utf-8")
    original = json.loads(fixture["registry"].read_text(encoding="utf-8"))
    ...
    record = _promote(fixture)
    assert not validate_document(record, "specialist_champion_transaction")
    assert record["install_kind"] == "first_role_install"
    assert record["incumbent_key"] == GENESIS_INCUMBENT_KEY
    assert record["incumbent_previous_role"] == GENESIS_PREVIOUS_ROLE
    assert record["incumbent_previous_lifecycle_state"] == GENESIS_PREVIOUS_LIFECYCLE
    assert record["incumbent_checkpoint_sha256"] == GENESIS_CHECKPOINT_SHA256
    promoted = json.loads(fixture["registry"].read_text(encoding="utf-8"))
    roles = {row["key"]: (row["role"], row["lifecycle_state"]) for row in promoted["models"]}
    assert roles == {"candidate_hand": ("champion_hand", "promoted")}
    ...
    assert rollback["serving_smoke"]["model_key"] == GENESIS_INCUMBENT_KEY
    assert rollback["serving_smoke"]["smoke"] == "first_role_install_absent"
    assert json.loads(fixture["registry"].read_text(encoding="utf-8")) == original


def test_first_role_install_rejects_non_genesis_packet_binding(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    fixture = _fixture(tmp_path, monkeypatch)
    document = json.loads(fixture["registry"].read_text(encoding="utf-8"))
    ...
    with pytest.raises(ModelRegistryError, match="genesis incumbent sentinel"):
        _promote(fixture)

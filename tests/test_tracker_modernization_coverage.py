...
def test_expanded_tracker_has_exact_authoritative_item_count_and_no_duplicate_ids() -> None:
    ...
    assert module["EXPECTED_ITEM_COUNT"] == 906
    assert len(items) == 906
    assert len(set(items)) == 906
...
def test_counts_and_owner_override_do_not_drift() -> None:
    ...
    assert "Total items: 906" in master
    assert "all 906 action items" in readme

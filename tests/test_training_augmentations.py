...
from maskfactory.training.dataset import (
    ...
    _mmseg_weight_authority,
    _validated_base_dataset_kwargs,
    mmseg_dataset_config,
)
from maskfactory.training.weighted_segmentor import weighted_mean_loss_dicts
...
def test_dataset_adapter_reproduces_exact_truth_tier_training_weight(tmp_path: Path) -> None:
    sample_id = "img_a3f9c2e17b04_p0"
    (tmp_path / "part_seg/images").mkdir(parents=True)
    (tmp_path / "part_seg/annotations").mkdir(parents=True)
    (tmp_path / "material_seg/annotations").mkdir(parents=True)
    Image.new("RGB", (4, 4), "gray").save(tmp_path / f"part_seg/images/{sample_id}.png")
    Image.fromarray(np.zeros((4, 4), dtype=np.uint8)).save(
        tmp_path / f"part_seg/annotations/{sample_id}.png"
    )
    Image.fromarray(np.zeros((4, 4), dtype=np.uint8)).save(
        tmp_path / f"material_seg/annotations/{sample_id}.png"
    )
    (tmp_path / "train.txt").write_text(sample_id + "\n", encoding="utf-8")
    (tmp_path / "sample_weights.json").write_text(
        json.dumps(
            {
    ...
    dataset = MaskFactorySegmentationDataset(tmp_path, "train")
    sample = dataset.load_sample(0)
    assert sample["truth_tier"] == "autonomous_certified_gold"
    assert sample["training_loss_weight"] == 0.65
    assert mmseg_dataset_config(tmp_path, "train", "part")["sample_weights_file"] == (
        "sample_weights.json"
    )
...
def test_mmseg_weight_authority_and_weighted_loss_are_exact(tmp_path: Path) -> None:
    sample_ids = ("img_anchor_p0", "img_auto_p0", "img_pseudo_p0")
    ...
    authority = _mmseg_weight_authority(
        {
            "data_root": str(tmp_path),
            "ann_file": "train.txt",
            "sample_weights_file": "sample_weights.json",
        }
    )
    assert [authority[sample_id]["training_loss_weight"] for sample_id in sample_ids] == [
        1.0,
        0.65,
        0.2,
    ]
    combined = weighted_mean_loss_dicts(
        [{"decode.loss": 1.0}, {"decode.loss": 3.0}, {"decode.loss": 9.0}],
        [1.0, 0.65, 0.2],
    )
    assert combined["decode.loss"] == pytest.approx((1.0 + 1.95 + 1.8) / 1.85)
    with pytest.raises(ValueError, match="finite"):
        weighted_mean_loss_dicts([{"loss": 1.0}], [0.0])

...
def test_runtime_evaluator_accepts_runpod_ada_sm89_when_listed_in_lock() -> None:
    lock = load_training_stack_lock()
    assert [8, 9] in lock["runtime"]["accepted_compute_capabilities"]
    versions = {name: item["version"] for name, item in lock["packages"].items()}
    ready = evaluate_openmmlab_runtime(
        versions=versions,
        torch_version="2.11.0+cu128",
    ...
    assert ready.ready
    rejected = evaluate_openmmlab_runtime(
        versions=versions,
        torch_version="2.11.0+cu128",
        mmcv_ops_loaded=True,
        datasets_registered=True,
    ...
    assert any("CUDA capability mismatch" in issue for issue in rejected.issues)

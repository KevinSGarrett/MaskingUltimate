...
def test_vlm_config_is_local_private_and_non_authoritative() -> None:
    ...
    assert config["runtime"]["cloud_enabled"] is False
    assert config["runtime"]["gpu_slot"] == "exclusive"
    assert config["models"]["primary_vlm"] == "qwen2.5vl:7b"
    assert config["models"]["fallback_vlm"] == "llama3.2-vision:11b"
    ...
    assert config["governance"]["source_images_leave_machine"] is False

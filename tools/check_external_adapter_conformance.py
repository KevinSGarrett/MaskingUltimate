"""Evaluate external Main adapter conformance against producer boundary policy."""
...
def main() -> int:
    ...
    evidence = build_external_adapter_conformance_evidence(observation, decided_at=args.decided_at)
    issues = validate_external_adapter_conformance_evidence(evidence)
    if issues:
        raise ExternalAdapterConformanceError("; ".join(issues))

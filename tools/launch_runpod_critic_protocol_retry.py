"""Launch one protocol-corrected Qwen 27B restart qualification."""
...
base=/workspace/maskfactory/runtime_artifacts/visual_critic_qwen27_protocol_retry
model=/workspace/models/visual_critics/qwen3_6_27b_fp8
mkdir -p "$base"
if [ ! -f "$model/model.safetensors.index.json" ]; then
  echo '{"status":"MODEL_NOT_READY"}'
  exit 0
fi
if [ -s "$base/retry.pid" ]; then
  old_pid=$(cat "$base/retry.pid")
  if kill -0 "$old_pid" 2>/dev/null; then
    python3 - "$old_pid" "$base" <<'PY'
import hashlib,json,pathlib,sys
pid=int(sys.argv[1]);base=pathlib.Path(sys.argv[2])
print(json.dumps({'status':'ALREADY_RUNNING','pid':pid,'job_dir':str(base),'script_sha256':hashlib.sha256((base/'retry.sh').read_bytes()).hexdigest()}))
PY
    exit 0
  fi
fi
...
content.append({'type':'text','text':'/no_think\nReturn a JSON object. Set verdict to uncertain and provide a short summary for these synthetic diagnostic panels.'})
schema={'name':'critic_smoke','strict':True,'schema':{'type':'object','properties':{'verdict':{'type':'string','enum':['uncertain']},'summary':{'type':'string','minLength':1}},'required':['verdict','summary'],'additionalProperties':False}}
payload={'model':'qwen3_6_27b_fp8','messages':[{'role':'user','content':content}],'temperature':0,'seed':1337,'max_tokens':96,'response_format':{'type':'json_schema','json_schema':schema}}
responses=[];latencies=[]
for replay in range(2):
    ...
    pathlib.Path(str(output)+f'.response{replay+1}.json').write_text(json.dumps(body,sort_keys=True)+'\n',encoding='utf-8')
    message=body['choices'][0]['message']
    text=str(message.get('content') or '').strip().removeprefix('```json').removesuffix('```').strip()
    parsed=json.loads(text)
    if set(parsed) != {'verdict','summary'} or parsed['verdict'] != 'uncertain': raise RuntimeError('Qwen response contract mismatch')
...
base=/workspace/maskfactory/runtime_artifacts/visual_critic_qwen27_protocol_retry
model=/workspace/models/visual_critics/qwen3_6_27b_fp8
exec 9>"$base/retry.lock"
flock -n 9 || exit 73
state() { python3 - "$1" "$2" <<'PY'
import json,pathlib,sys
path=pathlib.Path('/workspace/maskfactory/runtime_artifacts/visual_critic_qwen27_protocol_retry/state.json')
...
finish() { code=$?; cleanup_server; if [ "$code" -ne 0 ]; then state failed "exit_$code"; fi; exit "$code"; }
trap finish EXIT
export HF_HOME=/workspace/.cache/huggingface TRANSFORMERS_OFFLINE=1 HF_HUB_OFFLINE=1 TOKENIZERS_PARALLELISM=false
state qualification running
for run in 1 2; do
  python3 - <<'PY'
import socket
s=socket.socket(); busy=s.connect_ex(('127.0.0.1',18001)) == 0; s.close(); raise SystemExit(74 if busy else 0)
PY
  setsid "$setup/qwen36_env/bin/vllm" serve "$model" \
    ...
    --generation-config vllm >"$base/server${run}.log" 2>&1 &
  server_pid=$!
  ready=0
...
base=pathlib.Path('/workspace/maskfactory/runtime_artifacts/visual_critic_qwen27_protocol_retry')
previous=pathlib.Path('/workspace/maskfactory/runtime_artifacts/visual_critic_qualification')
specs={
 'qwen3_6_27b_fp8':{'family_id':'qwen','repository':'Qwen/Qwen3.6-27B-FP8','revision':'e89b16ebf1988b3d6befa7de50abc2d76f26eb09','quantization':'fp8','endpoint':'http://127.0.0.1:18001','root':pathlib.Path('/workspace/models/visual_critics/qwen3_6_27b_fp8'),'run_paths':[base/'run1.json',base/'run2.json']},
 'internvl3_5_8b_bf16':{'family_id':'internvl','repository':'OpenGVLab/InternVL3_5-8B','revision':'9bb6a56ad9cc69db95e2d4eeb15a52bbcac4ef79','quantization':'bf16','endpoint':'local-process://isolated','root':pathlib.Path('/workspace/models/visual_critics/internvl3_5_8b_bf16'),'run_paths':[previous/'internvl_run1.json',previous/'internvl_run2.json']},
}
prompt='Return only JSON with keys verdict and summary. Use verdict uncertain for these synthetic diagnostic panels.'
models=[]
for model_id,raw in specs.items():
 spec=dict(raw); root=spec.pop('root'); run_paths=spec.pop('run_paths')
 rows=[]
...
trap - EXIT
RETRY
chmod 700 "$base/retry.sh"
nohup bash "$base/retry.sh" >"$base/stdout.log" 2>"$base/stderr.log" </dev/null &
pid=$!
printf '%s\n' "$pid" > "$base/retry.pid"
python3 - "$pid" "$base" <<'PY'
import hashlib,json,pathlib,sys
pid=int(sys.argv[1]);base=pathlib.Path(sys.argv[2])
print(json.dumps({'status':'STARTED','pid':pid,'job_dir':str(base),'script_sha256':hashlib.sha256((base/'retry.sh').read_bytes()).hexdigest()}))
PY
"""
...
def run_remote_launch(*, host: str, port: int) -> dict[str, Any]:
    ...
    if completed.returncode != 0:
        ...
        raise RunPodVisualAuditError(f"RunPod protocol retry failed: {error[:300]}")
    lines = [line for line in completed.stdout.decode(errors="replace").splitlines() if line]
    if not lines:
        raise RunPodVisualAuditError("RunPod protocol retry returned no JSON")
    result = json.loads(lines[-1])
    if result.get("status") not in {"STARTED", "ALREADY_RUNNING", "MODEL_NOT_READY"}:
        raise RunPodVisualAuditError("RunPod protocol retry returned an invalid status")
    return result

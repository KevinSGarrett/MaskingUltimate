"""Isolated Main-side consumer climb3 runner (MF-P6-11.02/11.07 deepened matrices).

Kevin's mandate, Unblock 3: `C:\\Comfy_UI_Main` is an unrelated active Wave64
project with a dirty tree â€” we must NOT commit MaskFactory into it. Instead this
...
def run_failure_control() -> dict[str, Any]:
    ...
    incoherent_main_retry_rejected = inco_ev.get("admission", {}).get(
...
def run_final_release_handoff_firewall() -> dict[str, Any]:
    ...
    evidence that the honest firewall holds â€” it does NOT close the profile.
    """

...
def _process_group(
    ...
    cross_bundle["request"]["subject"]["scene_instance_id"] = packages[_name(1)][
        "scene_instance_id"
    ]
...
def run_local_multi_person_mode_a_group_slice(
    ...
    *,
    ...
    groups = _select_groups(annotations_root, sizes, limit_per_size, prefer_contact=prefer_contact)
    if not groups:
        raise RuntimeError(f"no LV-MHP groups found for sizes={sizes}")

    workdir = Path(workdir)
    workdir.mkdir(parents=True, exist_ok=True)
    records = [_process_group(group, images_root, workdir, decided_at) for group in groups]

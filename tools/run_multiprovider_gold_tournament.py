...
``configs/multiprovider_tournament_families.yaml`` ΓÇö smokes alone are not enough;
...
# Cap working resolution so 4KΓÇô6K gold-volume sources cannot OOM the 8 GiB GPU
...
class Sam2LocalCudaRunner:
    """Governed local-CUDA SAM2.1 large with BiRefNet box prior (OOM ΓåÆ base_plus)."""
    ...
    def infer(self, image_path: Path, prior_mask: np.ndarray) -> np.ndarray:
        ...
        except Exception as exc:  # noqa: BLE001 ΓÇö OOM / load fall back once

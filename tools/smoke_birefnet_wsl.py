...
def run(checkpoint: Path, image_path: Path) -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(prefix="maskfactory-birefnet-") as temporary:
        ...
        # Windows cannot create unprivileged symlinks; copy is portable and matches
        # the governed local-CUDA ComfyUI invoke path.
        shutil.copy2(checkpoint.resolve(), model_dir / "model.safetensors")
